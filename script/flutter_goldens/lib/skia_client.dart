...
// If you are here trying to figure out how to use golden files in relevant
...
/// Flutter Packages Gold Dashboard.
class SkiaGoldClient {
  ...
  /// A client for making Http requests to the Flutter Packages Gold dashboard.
  ...
  /// service.
  ///
  /// Since tests are executed in parallel, and in random
  /// order, this will signal if this instance of the Gold client has been
  /// initialized.
  ...
  Future<void> imgtestInit() async {
    ...
    final io.ProcessResult result = await process.run(imgtestInitCommand.cast<String>());
  ...
  Future<bool> imgtestAdd(String testName, File goldenFile) async {
    ...
    if (result.exitCode != 0) {
      ...
      final File resultFile = workDirectory.childFile(fs.path.join('temp', 'result-state.json'));
      ...
      final buf = StringBuffer()
        ...
        ..writeln('testing. Golden file images in flutter/packages are triaged ')
  ...
  /// Packages Gold at head.
  Future<String?> getExpectationForTest(String testName) async {
    late String? expectation;
    final String traceID = getTraceID(testName);
    final Uri requestForExpectations = Uri.parse(
      'https://flutter-packages-gold.skia.org/json/v2/latestpositivedigest/$traceID',
    );
  ...
  /// Flutter Packages Gold dashboard.
  ///
  /// The provided image hash represents an expectation from Flutter Packages Gold.
  Future<List<int>> getImageBytes(String imageHash) async {
    final imageBytes = <int>[];
    final Uri requestForImage = Uri.parse(
      'https://flutter-packages-gold.skia.org/img/images/$imageHash.png',
    );
    final io.HttpClientRequest request = await httpClient.getUrl(requestForImage);
    final io.HttpClientResponse response = await request.close();
    if (response.statusCode != io.HttpStatus.ok) {
      throw SkiaException(
        'Failed to retrieve image bytes from Gold for hash $imageHash. '
  ...
  List<String> getCIArguments() {
    final String? streamPrefix = platform.environment['LOGDOG_STREAM_PREFIX'];
    if (streamPrefix == null) {
      throw const SkiaException('LOGDOG_STREAM_PREFIX environment variable is missing.');
    }
    final String jobId = streamPrefix.split('/').last;
