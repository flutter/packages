...
import 'dart:io' as io show HttpClient, IOException, OSError, SocketException;
...
// If you are here trying to figure out how to use golden files for Flutter
// repos, consider reading this documentation:
...
Future<void> testExecutable(FutureOr<void> Function() testMain, {String? namePrefix}) async {
  ...
  if (FlutterPostSubmitFileComparator.isForEnvironment(platform)) {
    ...
    goldenFileComparator = FlutterSkippingFileComparator.fromLocalFileComparator(
      ...
      'flutter/packages, or in test shards that are not configured for using goldctl.',
      platform: platform,
      namePrefix: namePrefix,
      log: print,
      fs: fs,
...
/// repository.
///
/// Golden file testing for the `flutter/packages` repository is handled by three
...
///     testing, after a pull request has landed on the main branch. This
///     comparator uses the [SkiaGoldClient] and the `goldctl` tool to upload
///     tests to the [Flutter Packages Gold dashboard](https://flutter-packages-gold.skia.org).
///     Flutter Gold manages the master golden files for the packages that use
///     matchesGoldenFile in the `flutter/packages` repository.
///
///   * The [FlutterPreSubmitFileComparator] is utilized in pre-submit testing,
///     before a pull request lands on the main branch. This
...
///     from [Flutter Packages Gold](https://flutter-packages-gold.skia.org) and
///     manually compare pixels. If a difference is detected, this comparator will
...
abstract class FlutterGoldenFileComparator extends GoldenFileComparator {
  ...
  Uri _addPrefix(Uri golden) {
    ...
    assert(
      ...
      'Golden files in flutter/packages must end with the file extension '
      '.png.',
    );
...
/// images for package-related golden tests and process results.
///
/// See also:
///
...
class FlutterPostSubmitFileComparator extends FlutterGoldenFileComparator {
  ...
  Future<bool> compare(Uint8List imageBytes, Uri golden) async {
    try {
      ...
      // TODO(Piinks): Throw TestFailure once the Flutter Packages Gold dashboard is set up.
      log('$e');
      return true;
    }
  }
...
/// outside of the flutter/packages repository.
///
/// See also:
///
...
class FlutterSkippingFileComparator extends FlutterGoldenFileComparator {
  ...
  static FlutterSkippingFileComparator fromLocalFileComparator(
    ...
    final skiaClient = SkiaGoldClient(
      fs.directory(fs.path.fromUri(basedir)),
      platform: platform,
      log: log,
      fs: fs,
      process: process,
  ...
  static Future<FlutterGoldenFileComparator> fromLocalFileComparator({
    ...
    try {
      ...
      return FlutterSkippingFileComparator(
        baseDirectory.uri,
        goldens,
        'IOException occurred, could not reach Gold. '
        'Switching to FlutterSkippingGoldenFileComparator.',
        namePrefix: namePrefix,
        platform: platform,
        log: log,
