...
''');
    ...
    runner = CommandRunner<void>('validate_repo_info_test', 'Test for validating repo info');
...
''';
  ...
  String readmeTableEntry(String packageName, {String repoName = 'flutter/packages'}) {
...
''');
  ...
  test('passes when no .ci.yaml is present if nothing uses batched release', () async {
    final packages = <RepositoryPackage>[createFakePackage('a_package', packagesDir)];
...
''');
  ...
  group('custom package labels', () {
    test('passes for correct custom package label in README', () async {
      ...
      setToolConfig(root, packageLabels: <String, String>{'a_package': customLabel});
...
''');
    ...
    test('fails when default package label is used in README but custom is configured', () async {
      final packages = <RepositoryPackage>[createFakePackage('a_package', packagesDir)];
...
''');
      ...
      expect(commandError, isA<ToolExit>());
      expect(output, contains(contains('Incorrect link in root README.md table')));
    });
  });
