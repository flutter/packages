...
''');
    ...
    test('saves a screenshot if test is taking too long', () async {
      ...
      processRunner.mockProcessesForExecutable['flutter']!.addAll(<FakeProcessInfo>[
        FakeProcessInfo(_MockDelayingProcess(delayDuration: const Duration(minutes: 11), fakeAsync: fakeAsync), <String>['test']),
      ...
      expect(
        ...
        containsAllInOrder(<Matcher>[
          contains('Running for plugin'),
          contains('Test is taking a long time, taking screenshot...'),
          contains('No issues found!'),
        ]),
      );
...
class _MockDelayingProcess extends Mock implements io.Process {
  ...
  _MockDelayingProcess({required Duration delayDuration, required FakeAsync fakeAsync}) : _delayDuration = delayDuration, _fakeAsync = fakeAsync;
