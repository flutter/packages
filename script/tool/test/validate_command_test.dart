...
void main() {
  ...
  setUp(() {
    ...
    final command = ValidateCommand(packagesDir, gitDir: gitDir);
    runner = CommandRunner<void>('validate_test', 'Test for $ValidateCommand');
...
something: true
    ''');
        ...
        expect(
          ...
          containsAllInOrder(<Matcher>[
            contains(
              'Unknown key `something` in config, the possible keys are',
            ),
          ]),
        );
      });
      ...
      test(
        'fails for invalid value type for batch property in release',
        () async {
          final RepositoryPackage package = createFakePackage(
            'a_package',
            packagesDir,
