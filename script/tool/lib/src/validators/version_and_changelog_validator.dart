...
class VersionAndChangelogValidator {
  ...
  /// package that uses batch release.
  static const String _missingChangelogOverrideLabel = 'override: allow CHANGELOG.md edit';
...
''');
  ...
  Future<bool> _validatePendingChangeForBatchReleasePackage({
    ...
    if (changedFilesInPackage.contains('$relativePackagePath/CHANGELOG.md')) {
      if (_prLabels.contains(_missingChangelogOverrideLabel)) {
        _logWarning(
          'Allowing CHANGELOG.md update due to the '
          '"$_missingChangelogOverrideLabel" label.',
        );
      } else {
        printError(
