...
@interface FGMPlatformPointOfInterest ()
+ (FGMPlatformPointOfInterest *)fromList:(NSArray<id> *)list;
+ (nullable FGMPlatformPointOfInterest *)nullableFromList:(NSArray<id> *)list;
- (NSArray<id> *)toList;
@end
...
@implementation FGMPlatformPointOfInterest
+ (instancetype)makeWithPosition:(FGMPlatformLatLng *)position
    name:(NSString *)name
    placeId:(NSString *)placeId {
  FGMPlatformPointOfInterest* pigeonResult = [[FGMPlatformPointOfInterest alloc] init];
  pigeonResult.position = position;
...
+ (FGMPlatformPointOfInterest *)fromList:(NSArray<id> *)list {
  FGMPlatformPointOfInterest *pigeonResult = [[FGMPlatformPointOfInterest alloc] init];
  pigeonResult.position = GetNullableObjectAtIndex(list, 0);
  pigeonResult.name = GetNullableObjectAtIndex(list, 1);
  pigeonResult.placeId = GetNullableObjectAtIndex(list, 2);
  return pigeonResult;
}
+ (nullable FGMPlatformPointOfInterest *)nullableFromList:(NSArray<id> *)list {
  return (list) ? [FGMPlatformPointOfInterest fromList:list] : nil;
}
- (NSArray<id> *)toList {
  return @[
    self.position ?: [NSNull null],
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 151: 
      return [FGMPlatformPointOfInterest fromList:[self readValue]];
    case 152: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FGMPlatformMapTypeBox class]]) {
    ...
    [self writeByte:150];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[FGMPlatformPointOfInterest class]]) {
    [self writeByte:151];
    [self writeValue:[value toList]];
...
- (void)didTapPointOfInterest:(FGMPlatformPointOfInterest *)arg_poi completion:(void (^)(FlutterError *_Nullable))completion {
  NSString *channelName = [NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.MapsCallbackApi.onPoiTap", _messageChannelSuffix];
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:channelName
      binaryMessenger:self.binaryMessenger
      codec:FGMGetGoogleMapsFlutterPigeonMessagesCodec()];
  [channel sendMessage:@[arg_poi ?: [NSNull null]] reply:^(NSArray<id> *reply) {
    if (reply != nil) {
      if (reply.count > 1) {
        completion([FlutterError errorWithCode:reply[0] message:reply[1] details:reply[2]]);
      } else {
