...
+ (FGMPlatformMapViewCreationParams *)fromList:(NSArray<id> *)list {
  ...
  FGMPlatformMarkerTypeBox *boxedFGMPlatformMarkerType = GetNullableObjectAtIndex(list, 9);
...
+ (instancetype)makeWithBackgroundColor:(nullable NSNumber *)backgroundColor
    ...
    glyphColor:(nullable NSNumber *)glyphColor
    glyphTextColor:(nullable NSNumber *)glyphTextColor
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 130: {
      NSNumber *enumAsNumber = [self readValue];
      return enumAsNumber == nil ? nil : [[FGMMarkerCollisionBehaviorBox alloc] initWithValue:[enumAsNumber integerValue]];
    }
    case 131: {
      NSNumber *enumAsNumber = [self readValue];
...
NSObject<FlutterMessageCodec> *FGMGetMessagesCodec(void) {
  ...
  dispatch_once(&sPred, ^{
    FGMMessagesPigeonCodecReaderWriter *readerWriter = [[FGMMessagesPigeonCodecReaderWriter alloc] init];
  ...
  /// Moves the camera according to [cameraUpdate], animating the update.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FGMGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(animateCameraWithUpdate:error:)], @"FGMMapsApi api (%@) doesn't respond to @selector(animateCameraWithUpdate:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        FGMPlatformCameraUpdate *arg_cameraUpdate = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
