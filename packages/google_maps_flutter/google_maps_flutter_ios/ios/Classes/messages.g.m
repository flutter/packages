...
- (void)didChangeActiveLevel:(nullable FGMPlatformIndoorLevel *)arg_activeLevel
                  completion:(void (^)(FlutterError *_Nullable))completion {
  NSString *channelName = [NSString
      stringWithFormat:
          @"%@%@",
          @"dev.flutter.pigeon.google_maps_flutter_ios.MapsCallbackApi.onActiveLevelChanged",
          _messageChannelSuffix];
  FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel messageChannelWithName:channelName
                                         binaryMessenger:self.binaryMessenger
                                                   codec:FGMGetMessagesCodec()];
  [channel sendMessage:@[ arg_activeLevel ?: [NSNull null] ]
                 reply:^(NSArray<id> *reply) {
                   if (reply != nil) {
                     if (reply.count > 1) {
                       completion([FlutterError errorWithCode:reply[0]
