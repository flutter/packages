...
static void InterpretGroundOverlayOptions(NSDictionary* data, id<FLTGoogleMapGroundOverlayOptionsSink> sink,
  ...
  if (location) {
    if (height != nil) {
      ...
      if (width != nil) {
        [sink setLocation:[FLTGoogleMapJSONConversions locationFromLatLong:location] width:width.doubleValue height:150];
      }
    }
  }
...
- (void)addGroundOverlays:(NSArray*)groundOverlaysToAdd {
  for (NSDictionary *groundOverlay in groundOverlaysToAdd) {
    ...
    if (bounds) {
      CLLocationCoordinate2D location = [FLTGoogleMapJSONConversions locationFromLatLong:groundOverlay[@"location"]];
