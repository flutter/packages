...
void main() {
  ...
  testWidgets('testMapToolbar returns false', (WidgetTester tester) async {
    ...
    final int mapId = await mapIdCompleter.future;
    final GoogleMapsInspectorPlatform inspector = GoogleMapsInspectorPlatform.instance!;
    final bool mapToolbarEnabled = await inspector.isMapToolbarEnabled(mapId: mapId);
  ...
  testWidgets('updateMinMaxZoomLevels', (WidgetTester tester) async {
    ...
    MinMaxZoomPreference zoomLevel = await inspector.getMinMaxZoomLevels(mapId: controller.mapId);
  ...
  testWidgets('testZoomControlsEnabled', (WidgetTester tester) async {
    ...
    final int mapId = await mapIdCompleter.future;
    final GoogleMapsInspectorPlatform inspector = GoogleMapsInspectorPlatform.instance!;
    final bool zoomControlsEnabled = await inspector.areZoomControlsEnabled(mapId: mapId);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final ScreenCoordinate coordinate = await mapController.getScreenCoordinate(
        _kInitialCameraPosition.target,
      );
  ...
  testWidgets('testResizeWidget', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      Directionality(
        textDirection: TextDirection.ltr,
        child: MaterialApp(
          home: Scaffold(body: SizedBox(height: 100, width: 100, child: map)),
        ),
      ),
    );
    final ExampleGoogleMapController controller = await controllerCompleter.future;
    ...
    await tester.pumpWidget(
      Directionality(
        textDirection: TextDirection.ltr,
        child: MaterialApp(
          home: Scaffold(body: SizedBox(height: 400, width: 400, child: map)),
        ),
      ),
    );
  ...
  testWidgets('updating a marker does not hide its info window', (WidgetTester tester) async {
    final Key key = GlobalKey();
    const marker = Marker(
      markerId: MarkerId('marker'),
      infoWindow: InfoWindow(title: 'InfoWindow'),
    ...
    const clusterManager = ClusterManager(clusterManagerId: ClusterManagerId('cluster_manager'));
    ...
    for (final cm in clusterManagers) {
      for (var i = 0; i < markersPerClusterManager; i++) {
        ...
        final marker = Marker(
          ...
          position: LatLng(_kInitialMapCenter.latitude + i, _kInitialMapCenter.longitude),
        );
    ...
    final MarkerId markerIdToMove = markers.entries
        .firstWhere(
          (MapEntry<MarkerId, Marker> entry) =>
              entry.value.clusterManagerId == clusterManagers.first.clusterManagerId,
        )
        .key;
  ...
  testWidgets('advanced markers clustering', (WidgetTester tester) async {
    ...
    for (final MapEntry<MarkerId, AdvancedMarker> entry in markers.entries) {
      markers[entry.key] = _copyAdvancedMarkerWithClusterManagerId(entry.value, null);
    }
    await tester.pumpWidget(
      Directionality(
        textDirection: TextDirection.ltr,
  ...
  testWidgets('markerWithLegacyAsset', (WidgetTester tester) async {
    ...
    const imageConfiguration = ImageConfiguration(devicePixelRatio: 2.0, size: Size(100, 100));
    final markers = <Marker>{
      Marker(
        markerId: const MarkerId('1'),
  ...
  group('GroundOverlay', () {
    ...
    void expectGroundOverlayEquals(GroundOverlay source, GroundOverlay response) {
      ...
      expect(response.bearing, moreOrLessEquals(source.bearing, epsilon: _floatTolerance));
      ...
      expect(response.anchor?.dx, moreOrLessEquals(source.anchor!.dx, epsilon: _floatTolerance));
      expect(response.anchor?.dy, moreOrLessEquals(source.anchor!.dy, epsilon: _floatTolerance));
    }
      ...
      await tester.pumpWidget(
        Directionality(
          ...
          child: ExampleGoogleMap(
            key: key,
            initialCameraPosition: _kInitialCameraPosition,
            groundOverlays: <GroundOverlay>{groundOverlayBounds1, groundOverlayPosition1},
            onMapCreated: (ExampleGoogleMapController controller) {
              mapIdCompleter.complete(controller.mapId);
            },
          ),
      ...
      await tester.pumpWidget(
        Directionality(
          ...
          child: ExampleGoogleMap(
            key: key,
            initialCameraPosition: _kInitialCameraPosition,
            groundOverlays: <GroundOverlay>{groundOverlayBounds1New, groundOverlayPosition1New},
            onMapCreated: (ExampleGoogleMapController controller) {
              fail('update: OnMapCreated should get called only once.');
            },
          ),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final CameraPosition beforeFinishedPosition = await inspector.getCameraPosition(
        mapId: controller.mapId,
      );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await controller.moveCamera(CameraUpdate.newCameraPosition(_kInitialCameraPosition));
      await tester.pumpAndSettle();
  ...
  testWidgets('markerWithPinConfig', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      Directionality(
        ...
        child: ExampleGoogleMap(
          initialCameraPosition: const CameraPosition(target: LatLng(10.0, 20.0)),
          markers: markers,
          markerType: MarkerType.advancedMarker,
          onMapCreated: (ExampleGoogleMapController controller) =>
              controllerCompleter.complete(controller),
...
Future<void> _checkCameraUpdateByType(
  ...
  switch (type) {
    ...
    case CameraUpdateType.zoomBy:
      expect(currentPosition.zoom, wrapMatcher(equals(_kInitialZoomLevel + _kTestZoomByAmount)));
