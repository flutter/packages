...
@interface FGMHeatmapController ()
...
@implementation FGMHeatmapController
- (instancetype)initWithHeatmap:(FGMPlatformHeatmap *)heatmap
  ...
  if (self) {
    ...
    [FGMHeatmapController updateHeatmap:_heatmapTileLayer
...
@interface FGMHeatmapsController ()

/// A map from heatmapId to the controller that manages it.
@property(nonatomic, strong)
    NSMutableDictionary<NSString *, FGMHeatmapController *> *heatmapIdToController;
...
@implementation FGMHeatmapsController
...
- (void)addHeatmaps:(NSArray<FGMPlatformHeatmap *> *)heatmapsToAdd {
  for (FGMPlatformHeatmap *heatmap in heatmapsToAdd) {
    ...
    FGMHeatmapController *controller =
        [[FGMHeatmapController alloc] initWithHeatmap:heatmap
...
- (void)changeHeatmaps:(NSArray<FGMPlatformHeatmap *> *)heatmapsToChange {
  for (FGMPlatformHeatmap *heatmap in heatmapsToChange) {
    FGMHeatmapController *controller = _heatmapIdToController[heatmap.heatmapId];
...
- (void)removeHeatmapsWithIdentifiers:(NSArray<NSString *> *)identifiers {
  for (NSString *heatmapId in identifiers) {
    FGMHeatmapController *controller = _heatmapIdToController[heatmapId];
    if (!controller) {
      continue;
    }
