...
* Fixes a bug where using `cloudMapId` for cloud-based styling would fail if the `style` property was also present (styles are now ignored when `cloudMapId` is set).
