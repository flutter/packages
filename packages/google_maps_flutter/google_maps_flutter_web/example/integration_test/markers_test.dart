  ...
  group(
    ...
    () {
      ...
      testWidgets('InfoWindow show/hide', (WidgetTester tester) async {
        ...
        expect(
            controller.markers[const MarkerId('1')]?.infoWindowShown, isFalse);

        controller.showMarkerInfoWindow(const MarkerId('1'));

        expect(
            controller.markers[const MarkerId('1')]?.infoWindowShown, isTrue);
      ...
      testWidgets('only single InfoWindow is visible',
        ...
        expect(
            controller.markers[const MarkerId('1')]?.infoWindowShown, isFalse);
        expect(
            controller.markers[const MarkerId('2')]?.infoWindowShown, isFalse);
        ...
        expect(
            controller.markers[const MarkerId('1')]?.infoWindowShown, isFalse);
        expect(
            controller.markers[const MarkerId('2')]?.infoWindowShown, isTrue);
      });
      ...
      testWidgets('markers with custom asset icon work',
        ...
        expect(controller.markers.length, 1);
        final gmaps.Icon? icon = controller
      ...
      testWidgets('InfoWindow content is clickable',
          (WidgetTester tester) async {
        final Set<Marker> markers = <Marker>{
          const Marker(
            markerId: MarkerId('1'),
        ...
        expect(
            (event as InfoWindowTapEvent).value, equals(const MarkerId('1')));
      });
    },
    skip: true,
  );
