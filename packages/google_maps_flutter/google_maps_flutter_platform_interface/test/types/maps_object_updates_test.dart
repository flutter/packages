...
void main() {
  ...
  group('tile overlay updates tests', () {
    test('Correctly set toRemove, toAdd and toChange', () async {
      const to1 = TestMapsObject(
        MapsObjectId<TestMapsObject>('id1'),
      );
      const to2 = TestMapsObject(
        MapsObjectId<TestMapsObject>('id2'),
      );
      const to3 = TestMapsObject(
      ...
      const to4 = TestMapsObject(
        MapsObjectId<TestMapsObject>('id4'),
      );
      final previous = <TestMapsObject>{to1, to2, to3};
      final current = <TestMapsObject>{
        to2,
        to3Changed,
        to4,
      };
      final updates = TestMapsObjectUpdate.from(
        previous,
        current,
      );
    ...
    test('equality', () async {
      ...
      final previous = <TestMapsObject>{to1, to2, to3};
      final current1 = <TestMapsObject>{
        to2,
        to3Changed,
        to4,
      };
      final current2 = <TestMapsObject>{
        to2,
        to3Changed,
        to4,
      };
      final current3 = <TestMapsObject>{to2, to4};
      final updates1 = TestMapsObjectUpdate.from(
        previous,
        current1,
      );
      final updates2 = TestMapsObjectUpdate.from(
        previous,
        current2,
      );
      final updates3 = TestMapsObjectUpdate.from(
        previous,
        current3,
      );
