...
class FakeGoogleMapsFlutterPlatform extends GoogleMapsFlutterPlatform {
  ...
  Future<void> animateCamera(CameraUpdate cameraUpdate, {required int mapId}) async {
    mapInstances[mapId]?.animateCameraConfiguration = CameraUpdateWithConfiguration(
...
class PlatformMapStateRecorder {
  PlatformMapStateRecorder({
    ...
    heatmapUpdates.add(HeatmapUpdates.from(const <Heatmap>{}, mapObjects.heatmaps));
...
class CameraUpdateWithConfiguration {
  CameraUpdateWithConfiguration({required this.cameraUpdate, required this.configuration});
