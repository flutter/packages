...
final class UIImageIosPlatformImagesTests: XCTestCase {
  ...
  func testMultiResolutionImageUsesBest() {
    guard let image = flutterImageWithName(withName: "assets/multisize.png") else {
      return XCTFail("Image not found")
    }
  ...
  func testSingleResolutionFindsImage() {
    ...
    guard let image = flutterImageWithName(withName: "assets/monosize.png") else {
      return XCTFail("Image not found")
    }

    XCTAssertEqual(image.size.height, 125, accuracy: 0.00001)
  }

  func testMissingImageReturnsNil() {
    let image = flutterImageWithName(withName: "assets/no_such_image.png")
    return XCTAssertNil(image)
  }
}
