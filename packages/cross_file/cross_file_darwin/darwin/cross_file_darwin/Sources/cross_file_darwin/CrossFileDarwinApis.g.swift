...
protocol CrossFileDarwinApisPigeonProxyApiDelegate {
  /// An implementation of [PigeonApiAssetResourceManagerBytesListener] used to add a new Dart instance of
  /// `AssetResourceManagerBytesListener` to the Dart `InstanceManager` and make calls to Dart.
  func pigeonApiAssetResourceManagerBytesListener(_ registrar: CrossFileDarwinApisPigeonProxyApiRegistrar) -> PigeonApiAssetResourceManagerBytesListener
}
...
open class CrossFileDarwinApisPigeonProxyApiRegistrar {
  ...
  func setUp() {
    ...
    PigeonApiAssetResourceManagerBytesListener.setUpMessageHandlers(binaryMessenger: binaryMessenger, api: apiDelegate.pigeonApiAssetResourceManagerBytesListener(self))
  }
  func tearDown() {
    CrossFileDarwinApisPigeonInstanceManagerApi.setUpMessageHandlers(binaryMessenger: binaryMessenger, instanceManager: nil)
    PigeonApiAssetResourceManagerBytesListener.setUpMessageHandlers(binaryMessenger: binaryMessenger, api: nil)
  }
}
private class CrossFileDarwinApisPigeonInternalProxyApiCodecReaderWriter: FlutterStandardReaderWriter {
  ...
  private class CrossFileDarwinApisPigeonInternalProxyApiCodecWriter: CrossFileDarwinApisPigeonCodecWriter {
    ...
    override func writeValue(_ value: Any) {
      ...
      if let instance = value as? AssetResourceManagerBytesListener {
        pigeonRegistrar.apiDelegate.pigeonApiAssetResourceManagerBytesListener(pigeonRegistrar).pigeonNewInstance(
...
protocol PigeonApiDelegateAssetResourceManagerBytesListener {
  func pigeonDefaultConstructor(pigeonApi: PigeonApiAssetResourceManagerBytesListener) throws -> AssetResourceManagerBytesListener
  func openRead(pigeonApi: PigeonApiAssetResourceManagerBytesListener, pigeonInstance: AssetResourceManagerBytesListener, localIdentifier: String) throws -> Bool
}

protocol PigeonApiProtocolAssetResourceManagerBytesListener {
  func onDataReceived(pigeonInstance pigeonInstanceArg: AssetResourceManagerBytesListener, bytes bytesArg: FlutterStandardTypedData, completion: @escaping (Result<Void, PigeonError>) -> Void)
  func onCompletion(pigeonInstance pigeonInstanceArg: AssetResourceManagerBytesListener, error errorArg: String?, completion: @escaping (Result<Void, PigeonError>) -> Void)
}

final class PigeonApiAssetResourceManagerBytesListener: PigeonApiProtocolAssetResourceManagerBytesListener  {
  unowned let pigeonRegistrar: CrossFileDarwinApisPigeonProxyApiRegistrar
  let pigeonDelegate: PigeonApiDelegateAssetResourceManagerBytesListener
  init(pigeonRegistrar: CrossFileDarwinApisPigeonProxyApiRegistrar, delegate: PigeonApiDelegateAssetResourceManagerBytesListener) {
    self.pigeonRegistrar = pigeonRegistrar
    self.pigeonDelegate = delegate
  }
  static func setUpMessageHandlers(binaryMessenger: FlutterBinaryMessenger, api: PigeonApiAssetResourceManagerBytesListener?) {
    ...
    let pigeonDefaultConstructorChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.cross_file_darwin.AssetResourceManagerBytesListener.pigeon_defaultConstructor", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
...
withIdentifier: pigeonIdentifierArg)
    ...
    let openReadChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.cross_file_darwin.AssetResourceManagerBytesListener.openRead", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      openReadChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let pigeonInstanceArg = args[0] as! AssetResourceManagerBytesListener
        let localIdentifierArg = args[1] as! String
        do {
          let result = try api.pigeonDelegate.openRead(pigeonApi: api, pigeonInstance: pigeonInstanceArg, localIdentifier: localIdentifierArg)
  ...
  ///Creates a Dart instance of AssetResourceManagerBytesListener and attaches it to [pigeonInstance].
  func pigeonNewInstance(pigeonInstance: AssetResourceManagerBytesListener, completion: @escaping (Result<Void, PigeonError>) -> Void) {
      ...
      completion(
        .failure(
          PigeonError(
            ...
            message: "Error: Attempting to create a new Dart instance of AssetResourceManagerBytesListener, but the class has a nonnull callback method.", details: "")))
    }
  }
  func onDataReceived(pigeonInstance pigeonInstanceArg: AssetResourceManagerBytesListener, bytes bytesArg: FlutterStandardTypedData, completion: @escaping (Result<Void, PigeonError>) -> Void)   {
    if pigeonRegistrar.ignoreCallsToDart {
      ...
      completion(
        .failure(
          PigeonError(
            ...
            message: "Callback to `AssetResourceManagerBytesListener.onDataReceived` failed because native instance was not in the instance manager.", details: "")))
      return
    }
    let binaryMessenger = pigeonRegistrar.binaryMessenger
    let codec = pigeonRegistrar.codec
    let channelName: String = "dev.flutter.pigeon.cross_file_darwin.AssetResourceManagerBytesListener.onDataReceived"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    ...
    if pigeonRegistrar.ignoreCallsToDart {
      ...
      completion(
        .failure(
          PigeonError(
            code: "missing-instance-error",
            message: "Callback to `AssetResourceManagerBytesListener.onCompletion` failed because native instance was not in the instance manager.", details: "")))
      return
    }
    let binaryMessenger = pigeonRegistrar.binaryMessenger
    let codec = pigeonRegistrar.codec
    let channelName: String = "dev.flutter.pigeon.cross_file_darwin.AssetResourceManagerBytesListener.onCompletion"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
