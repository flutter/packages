...
abstract class PigeonApiInputStream(open val pigeonRegistrar: AndroidLibraryPigeonProxyApiRegistrar) {
   ...
   * returns them.
   */
  abstract fun readBytes(pigeon_instance: java.io.InputStream, len: Long, off: Long): dev.flutter.packages.cross_file_android.InputStreamReadBytesResponse
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val wrapped: List<Any?> = try {
              listOf(api.readBytes(pigeon_instanceArg, lenArg, offArg))
            } catch (exception: Throwable) {
              AndroidLibraryPigeonUtils.wrapError(exception)
            }
            reply.reply(wrapped)
