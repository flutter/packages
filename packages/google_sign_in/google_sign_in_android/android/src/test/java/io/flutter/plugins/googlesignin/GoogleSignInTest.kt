...
class GoogleSignInTest {
    ...
    @Mock
    var mockActivityPluginBinding: ActivityPluginBinding? = null

    @Mock
    var mockAuthorizationIntent: PendingIntent? = null

    @Mock
    var mockAuthorizationIntentSender: IntentSender? = null

    @Mock
    var mockCredentialManager: CredentialManager? = null

    @Mock
    var mockAuthorizationClient: AuthorizationClient? = null

    @Mock
    var mockGenericCredential: CustomCredential? = null

    @Mock
    var mockGoogleCredential: GoogleIdTokenCredential? = null

    @Mock
    var mockAuthorizationTask: Task<AuthorizationResult?>? = null

    @Mock
    var mockVoidTask: Task<Void?>? = null
    ...
    fun setUp() {
        ...
        Mockito.`when`<Task<Void?>?>(mockVoidTask!!.addOnSuccessListener(ArgumentMatchers.any<OnSuccessListener<in Void?>?>()))
            .thenReturn(mockVoidTask)
        Mockito.`when`<Task<Void?>?>(mockVoidTask!!.addOnFailureListener(ArgumentMatchers.any<OnFailureListener?>()))
            .thenReturn(mockVoidTask)
        Mockito.`when`<IntentSender?>(mockAuthorizationIntent!!.getIntentSender())
            .thenReturn(mockAuthorizationIntentSender)
        Mockito.`when`<Activity?>(mockActivityPluginBinding!!.getActivity())
    ...
    fun getCredential_returnsAuthenticationInfo() {
        ...
        plugin!!.getCredential(
            ...
            asCompatCallback<GetCredentialResult?> { reply: ResultCompat<GetCredentialResult?>? ->
                ...
                val credential =
                    (result as GetCredentialSuccess).credential
        ...
        val callbackCaptor =
            ArgumentCaptor.forClass<CredentialManagerCallback<GetCredentialResponse?, GetCredentialException?>?, CredentialManagerCallback<*, *>?>(
    ...
    fun getCredential_usesGetSignInWithGoogleOptionForButtonFlow() {
        ...
        plugin!!.getCredential(
            params,
            asCompatCallback<GetCredentialResult?> { reply: ResultCompat<GetCredentialResult?>? -> null })
        ...
        Mockito.verify<CredentialManager?>(mockCredentialManager)
            .getCredentialAsync(
                ...
                ArgumentMatchers.any<CancellationSignal?>(),
                ArgumentMatchers.any<Executor?>(),
                ArgumentMatchers.any<CredentialManagerCallback<GetCredentialResponse?, GetCredentialException?>?>()
            )

        Assert.assertEquals(1, captor.getValue()!!.credentialOptions.size.toLong())
        Assert.assertTrue(
            captor.getValue()!!.credentialOptions.get(0) is GetSignInWithGoogleOption
        )
    }
    ...
    fun getCredential_passesNonceInButtonFlow() {
        ...
        Assert.assertEquals(
            nonce,
            (captor.getValue()!!.credentialOptions.get(0) as GetSignInWithGoogleOption).nonce
        )
    }
    ...
    fun getCredential_reportsWrongCredentialType() {
        ...
        callbackCaptor
            .getValue()!!
            .onResult(GetCredentialResponse(PasswordCredential("wrong", "type")))
        Assert.assertTrue(callbackCalled[0]!!)
    }
    ...
    fun getCredential_reportsProviderConfigurationIssue() {
        ...
        plugin!!.getCredential(
            ...
            asCompatCallback<GetCredentialResult?> { reply: ResultCompat<GetCredentialResult?>? ->
                ...
                val failure = result as GetCredentialFailure
                Assert.assertEquals(
                    GetCredentialFailureType.PROVIDER_CONFIGURATION_ISSUE, failure.type
                )
                null
            })
    ...
    fun authorize_passesNullParamaters() {
        ...
        val params =
            PlatformAuthorizationRequest(scopes, null, null, null)

        Mockito.`when`<Task<AuthorizationResult?>?>(
            mockAuthorizationClient!!.authorize(
                ArgumentMatchers.any<AuthorizationRequest?>()
            )
        ).thenReturn(mockAuthorizationTask)
    ...
    fun authorize_passesOptionalParameters() {
        ...
        val params =
            PlatformAuthorizationRequest(scopes, hostedDomain, accountEmail, serverClientId)
    ...
    fun clearCredentialState_reportsSuccess() {
        ...
        val callbackCaptor =
            ArgumentCaptor.forClass<CredentialManagerCallback<Void?, ClearCredentialException?>?, CredentialManagerCallback<*, *>?>(
                CredentialManagerCallback::class.java
            )
        Mockito.verify<CredentialManager?>(mockCredentialManager)
            .clearCredentialStateAsync(
                ArgumentMatchers.any<ClearCredentialStateRequest?>(ClearCredentialStateRequest::class.java),
                ArgumentMatchers.any<CancellationSignal?>(),
                ArgumentMatchers.any<Executor?>(),
                callbackCaptor.capture()
            )
    ...
    fun revokeAccess_callsClient() {
        ...
        Mockito.`when`<Task<Void?>?>(mockAuthorizationClient!!.revokeAccess(ArgumentMatchers.any<RevokeAccessRequest?>()))
            .thenReturn(mockVoidTask)
        plugin!!.revokeAccess(
            params,
            asCompatCallback<Unit?> { reply: ResultCompat<Unit?>? -> null })

        val requestCaptor =
            ArgumentCaptor.forClass<RevokeAccessRequest?, RevokeAccessRequest?>(RevokeAccessRequest::class.java)
        Mockito.verify<AuthorizationClient?>(mockAuthorizationClient)
    ...
    fun clearAuthorizationToken_callsClient() {
        ...
        Mockito.`when`<Task<Void?>?>(mockAuthorizationClient!!.clearToken(ArgumentMatchers.any<ClearTokenRequest?>()))
            .thenReturn(mockVoidTask)
        ...
        val authRequestCaptor =
            ArgumentCaptor.forClass<ClearTokenRequest?, ClearTokenRequest?>(ClearTokenRequest::class.java)
        Mockito.verify<AuthorizationClient?>(mockAuthorizationClient)
    ...
    private fun mockSuccessAuthorizationResult(
        serverAuthCode: String?, accessToken: String?, scopes: MutableList<String?>?
    ): AuthorizationResult {
