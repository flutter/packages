    ...
    switch (mode) {
        ...
        switch (current) {
          ...
          default:
            throw ParserException(
              'Unexpected character ${_describeRune(current)}',
              line,
              column,
            );
        }
      ...
      case _TokenizerMode.minus: // "-"
        ...
        switch (current) {
          case -1:
            throw ParserException(
              'Unexpected end of file after minus sign',
              line,
              column,
            );
          case 0x30: // U+0030 DIGIT ZERO character (0)
      ...
      case _TokenizerMode.integerOnly:
        switch (current) {
          ...
          case 0x39: // U+0039 DIGIT NINE character (9)
            buffer.add(
              current,
            ); // https://github.com/dart-lang/sdk/issues/53349
          default:
            throw ParserException(
      ...
      case _TokenizerMode.numericDot: // "0.", "-0.", "00.", "1.", "-00."
        switch (current) {
          case -1:
            throw ParserException(
              'Unexpected end of file after decimal point',
              line,
              column,
            );
          case 0x30: // U+0030 DIGIT ZERO character (0)
      ...
      case _TokenizerMode.fraction: // "0.0", "-0.0", "00.0", "1.0", "-00.0"
        switch (current) {
          case -1:
            yield _DoubleToken(
              double.parse(String.fromCharCodes(buffer)),
              line,
              column,
              start,
              index - 1,
      ...
      case _TokenizerMode.x: // "0x", "0X"
        switch (current) {
          case -1:
            throw ParserException(
              'Unexpected end of file after 0x prefix',
              line,
              column,
            );
          case 0x30: // U+0030 DIGIT ZERO character (0)
      ...
      case _TokenizerMode.dot2: // ".."
        switch (current) {
          case -1:
            throw ParserException(
              'Unexpected end of file inside "..." symbol',
              line,
              column,
            );
          case 0x2E: // U+002E FULL STOP character (.)
            yield _SymbolToken(
              _SymbolToken.tripleDot,
              line,
              column,
              start,
              index,
      ...
      case _TokenizerMode.quoteEscapeUnicode1:
        switch (current) {
          case -1:
            throw ParserException(
              'Unexpected end of file inside Unicode escape',
              line,
              column,
            );
          case 0x30: // U+0030 DIGIT ZERO character (0)
...
class _Parser {
  ...
  T _withSourceRange<T extends BlobNode>(T node, SourceLocation? start) {
    if (sourceIdentifier != null && start != null) {
      node.associateSource(
        SourceRange(start, SourceLocation(sourceIdentifier!, _previousEnd)),
      );
    }
    return node;
  }
    ...
    while (_source.current is! _SymbolToken) {
      ...
      if (_foundIdentifier('default')) {
        ...
        if (cases.containsKey(key)) {
          throw ParserException._fromToken(
            'Switch has duplicate cases for key $key',
            _source.current,
          );
        }
      }
  ...
  Object _readValue({
    ...
    if (_source.current is _SymbolToken) {
      ...
      if (identifier == 'set') {
        ...
        final StateReference stateReference = _withSourceRange(
          StateReference(_readParts()),
          innerStart,
        );
      ...
      if (widgetBuilderScope.contains(identifier)) {
        ...
        _advance();
        return _withSourceRange(
          WidgetBuilderArgReference(identifier, _readParts()),
          start,
        );
      }
      ...
      if (index > 0) {
        ...
        return _withSourceRange(
          LoopReference(
            _loopIdentifiers.length - index,
            _readParts(optional: true),
          ),
          start,
        );
      }
  ...
  WidgetBuilderDeclaration _readWidgetBuilderDeclaration({
    ...
    if (widget is! ConstructorCall && widget is! Switch) {
      throw ParserException._fromToken(
        'Expecting a switch or constructor call got $widget',
        valueToken,
      );
    }
