...
class _BlobDecoder {
  ...
  int _readInt64() {
    ...
    final int a = bytes.getUint32(
      byteOffset,
      _blobEndian,
    ); // dead code on VM target
    final int b = bytes.getInt32(
      byteOffset + 4,
      _blobEndian,
    ); // dead code on VM target
  ...
  Switch _readSwitch() {
    ...
    final Map<Object?, Object> cases = Map<Object?, Object>.fromEntries(
      Iterable<MapEntry<Object?, Object>>.generate(
        count,
        (int index) =>
            MapEntry<Object?, Object>(_readSwitchKey(), _readArgument()),
      ),
    );
