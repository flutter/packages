...
Map<String, LocalWidgetBuilder> get _materialWidgetsDefinitions =>
    <String, LocalWidgetBuilder>{
      ...
      'ButtonBar': (BuildContext context, DataSource source) {
        ...
        if (ArgumentDecoders.enumValue<MainAxisSize>(
      ...
      'OverflowBar': (BuildContext context, DataSource source) {
        return OverflowBar(
          ...
          overflowAlignment: ArgumentDecoders.enumValue<OverflowBarAlignment>(
                  OverflowBarAlignment.values, source, ['overflowAlignment']) ??
      ...
      'DropdownButton': (BuildContext context, DataSource source) {
        ...
        final dropdownMenuItems = List<DropdownMenuItem<Object>>.generate(
          ...
          (int index) => DropdownMenuItem<Object>(
            ...
            value: source.v<String>(['items', index, 'value']) ??
            ...
            alignment: ArgumentDecoders.alignment(
                    source, ['items', index, 'alignment']) ??
        ...
        return DropdownButton<Object>(
          ...
          onChanged: source.handler(
              <Object>['onChanged'],
              (HandlerTrigger trigger) => (Object? value) =>
          ...
          icon: source.optionalChild(['icon']),
          iconDisabledColor:
              ArgumentDecoders.color(source, ['iconDisabledColor']),
          iconEnabledColor:
              ArgumentDecoders.color(source, ['iconEnabledColor']),
          iconSize: source.v<double>(['iconSize']) ?? 24.0,
          isDense: source.v<bool>(['isDense']) ?? false,
          isExpanded: source.v<bool>(['isExpanded']) ?? false,
          itemHeight:
              source.v<double>(['itemHeight']) ?? kMinInteractiveDimension,
      ...
      'InkResponse': (BuildContext context, DataSource source) {
        ...
        return InkResponse(
          ...
          onSecondaryTapUp: source.handler(['onSecondaryTapUp'],
              (VoidCallback trigger) => (TapUpDetails details) => trigger()),
          onSecondaryTapDown: source.handler(['onSecondaryTapDown'],
              (VoidCallback trigger) => (TapDownDetails details) => trigger()),
          onSecondaryTapCancel: source.voidHandler(['onSecondaryTapCancel']),
          onHighlightChanged: source.handler(['onHighlightChanged'],
              (VoidCallback trigger) => (bool highlighted) => trigger()),
          onHover: source.handler(['onHover'],
              (VoidCallback trigger) => (bool hovered) => trigger()),
          containedInkWell: source.v<bool>(['containedInkWell']) ?? false,
          highlightShape: ArgumentDecoders.enumValue<BoxShape>(
                  BoxShape.values, source, ['highlightShape']) ??
              BoxShape.circle,
          ...
          onFocusChange: source.handler(['onFocusChange'],
              (VoidCallback trigger) => (bool focus) => trigger()),
          autofocus: source.v<bool>(['autofocus']) ?? false,
          hoverDuration:
      ...
      'Material': (BuildContext context, DataSource source) {
        return Material(
          type: ArgumentDecoders.enumValue<MaterialType>(
                  MaterialType.values, source, ['type']) ??
              MaterialType.canvas,
          elevation: source.v<double>(['elevation']) ?? 0.0,
          color: ArgumentDecoders.color(source, ['color']),
          shadowColor: ArgumentDecoders.color(source, ['shadowColor']),
          surfaceTintColor:
              ArgumentDecoders.color(source, ['surfaceTintColor']),
          ...
          clipBehavior: ArgumentDecoders.enumValue<Clip>(
              ...
              Clip.none,
          animationDuration:
              ArgumentDecoders.duration(source, ['animationDuration'], context),
          child: source.child(['child']),
        );
      },
      ...
      'Slider': (BuildContext context, DataSource source) {
        ...
        final String label = labelText != null
