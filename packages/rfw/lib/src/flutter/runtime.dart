...
class Runtime extends ChangeNotifier {
  ...
  final Map<FullyQualifiedWidgetName, _ResolvedConstructor?> _cachedConstructors =
      <FullyQualifiedWidgetName, _ResolvedConstructor?>{};
  ...
  _ResolvedConstructor? _findConstructor(FullyQualifiedWidgetName fullName) {
    ...
    if (library is RemoteWidgetLibrary) {
      ...
      assert(
        library // ignore: prefer_void_to_null, type_check_with_null, https://github.com/dart-lang/sdk/issues/47017#issuecomment-907562014
  ...
  Object _bindArguments(
    ...
    if (node is SetStateHandler) {
      ...
      final BoundStateReference stateReference = (node.stateReference as StateReference).bind(
        stateDepth,
      );
...
abstract class _CurriedWidget extends BlobNode {
  const _CurriedWidget(this.fullName, this.arguments, this.widgetBuilderScope, this.initialState);
    ...
    if (node is DynamicMap) {
      return node.map<String, Object?>(
        (String name, Object? value) =>
            MapEntry<String, Object?>(name, _bindLoopVariable(value!, argument, depth)),
      );
    }
    if (node is DynamicList) {
      return List<Object>.generate(
  ...
  static _ResolvedDynamicList _listLookup(
    ...
    while ((effectiveIndex <= targetEffectiveIndex || targetEffectiveIndex < 0) &&
      ...
      if (current is DataReference) {
        ...
        if (current is EventHandler) {
          current = EventHandler(
            current.eventName,
            _fix(current.eventArguments, stateResolver, dataResolver, widgetBuilderArgResolver)
                as DynamicMap,
          );
...
class _WidgetState extends State<_Widget> implements DataSource {
  ...
  Widget? _fetchBuilder(List<Object> argsKey, DynamicMap builderArg, {bool optional = true}) {
    final Object value = _fetch(argsKey, expandLists: false);
    if (value is _RemoteWidgetBuilder) {
      final _CurriedWidget curriedWidget = value(builderArg);
      return curriedWidget.build(context, widget.data, widget.remoteEventTarget, widget.states);
    }
    return optional
        ? null
        : _buildErrorWidget(
