...
void main() {
  ...
  testWidgets('deepClone', (WidgetTester tester) async {
    final Map<String, Object> map = <String, Object>{
      'outer': <String, Object>{'inner': true},
    };
  ...
  testWidgets('Import loops', (WidgetTester tester) async {
    final Runtime runtime = Runtime()
      ..update(
        const LibraryName(<String>['a']),
        parseLibraryFile('''
        import b;
      '''),
      )
      ..update(
        const LibraryName(<String>['b']),
        parseLibraryFile('''
        import a;
      '''),
      );
  ...
  testWidgets('list lookup of esoteric values', (WidgetTester tester) async {
    ...
    await tester.pump();
    expect(
      tester.firstWidget<ColoredBox>(find.byType(ColoredBox)).color,
      const Color(0x00000002),
    );
  ...
  testWidgets('DataSource', (WidgetTester tester) async {
    ...
    runtime.update(
      ...
      LocalWidgetLibrary(<String, LocalWidgetBuilder>{
        'Test': (BuildContext context, DataSource source) {
          ...
          expect(source.childList(<Object>['a']), <Matcher>[
            isA<ErrorWidget>(),
          ]);
          expect(
            tester.takeException().toString(),
          ...
          expect(source.childList(<Object>['b']), <Matcher>[
            isA<ErrorWidget>(),
          ]);
          expect(
            tester.takeException().toString(),
    ...
    bool tested = false;
    tester.element(find.byType(ColoredBox)).visitAncestorElements((
      Element node,
    ) {
      expect(
        node.toString(),
    ...
    runtime.update(
      ...
      LocalWidgetLibrary(<String, LocalWidgetBuilder>{
        'Builder': (BuildContext context, DataSource source) {
          final Widget? builder = source.optionalBuilder(<String>[
            'builder',
          ], <String, Object?>{});
          return builder ??
