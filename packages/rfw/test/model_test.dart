...
void main() {
  testWidgets('$LibraryName', (WidgetTester tester) async {
    ...
    final LibraryName a = LibraryName(<String>[
      'core',
      deconst<String>('widgets'),
    ]);
    final LibraryName b = LibraryName(<String>[
      'core',
      deconst<String>('widgets'),
    ]);
    final LibraryName c = LibraryName(<String>[
      'core',
      deconst<String>('material'),
    ]);
  ...
  testWidgets('$FullyQualifiedWidgetName', (WidgetTester tester) async {
    const FullyQualifiedWidgetName aa = FullyQualifiedWidgetName(
      LibraryName(<String>['a']),
      'a',
    );
    const FullyQualifiedWidgetName ab = FullyQualifiedWidgetName(
      LibraryName(<String>['a']),
      'b',
    );
    const FullyQualifiedWidgetName bb = FullyQualifiedWidgetName(
      LibraryName(<String>['b']),
      'b',
    );
    expect('$aa', 'a:a');
  ...
  testWidgets('toStrings', (WidgetTester tester) async {
    ...
    expect(
      '${const BoundArgsReference(false, <Object>["a"])}',
      'args(false).a',
    );
    ...
    expect(
      '${const SetStateHandler(StateReference(<Object>["a"]), false)}',
      'set state.a = false',
    );
  ...
  testWidgets('$BoundArgsReference', (WidgetTester tester) async {
    ...
    final BoundArgsReference result = const ArgsReference(<Object>[
      0,
    ]).bind(target);
  ...
  testWidgets('$DataReference', (WidgetTester tester) async {
    final DataReference result = const DataReference(<Object>[
      0,
  ...
  testWidgets('$SourceLocation comparison', (WidgetTester tester) async {
    ...
    expect(test2 >= test1, isTrue);
    // map
    final Map<SourceLocation, SourceLocation> map =
        <SourceLocation, SourceLocation>{test1: test1, test2: test2};
    expect(map[test1], test1);
    expect(map[test2], test2);
  });

  testWidgets('$SourceLocation with non-matching sources', (
    WidgetTester tester,
  ) async {
