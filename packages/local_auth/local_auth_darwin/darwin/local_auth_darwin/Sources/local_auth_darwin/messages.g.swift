...
struct AuthStrings: Hashable {
  ...
  static func == (lhs: AuthStrings, rhs: AuthStrings) -> Bool {
    ...
    return deepEqualsmessages(lhs.reason, rhs.reason) && deepEqualsmessages(lhs.cancelButton, rhs.cancelButton) && deepEqualsmessages(lhs.localizedFallbackTitle, rhs.localizedFallbackTitle)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("AuthStrings")
    deepHashmessages(value: reason, hasher: &hasher)
    deepHashmessages(value: cancelButton, hasher: &hasher)
    deepHashmessages(value: localizedFallbackTitle, hasher: &hasher)
  }
}
...
struct AuthOptions: Hashable {
  ...
  static func == (lhs: AuthOptions, rhs: AuthOptions) -> Bool {
    ...
    return deepEqualsmessages(lhs.biometricOnly, rhs.biometricOnly) && deepEqualsmessages(lhs.sticky, rhs.sticky)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("AuthOptions")
    deepHashmessages(value: biometricOnly, hasher: &hasher)
    deepHashmessages(value: sticky, hasher: &hasher)
  }
}
...
struct AuthResultDetails: Hashable {
  ...
  static func == (lhs: AuthResultDetails, rhs: AuthResultDetails) -> Bool {
    ...
    return deepEqualsmessages(lhs.result, rhs.result) && deepEqualsmessages(lhs.errorMessage, rhs.errorMessage) && deepEqualsmessages(lhs.errorDetails, rhs.errorDetails)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("AuthResultDetails")
    deepHashmessages(value: result, hasher: &hasher)
    deepHashmessages(value: errorMessage, hasher: &hasher)
    deepHashmessages(value: errorDetails, hasher: &hasher)
  }
}
...
protocol LocalAuthApi {
  ...
  func stopAuthentication(completion: @escaping (Result<Bool, Error>) -> Void)
}
...
class LocalAuthApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: LocalAuthApi?, messageChannelSuffix: String = "") {
    ...
    /// Returns false if there was no authentication in progress,
    /// or an error occurred.
    let stopAuthenticationChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.local_auth_darwin.LocalAuthApi.stopAuthentication\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      stopAuthenticationChannel.setMessageHandler { _, reply in
        api.stopAuthentication { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      stopAuthenticationChannel.setMessageHandler(nil)
    }
  }
}
