 ...
 */
    ...
    @JvmField
    @VisibleForTesting
    val authInProgress: AtomicBoolean = AtomicBoolean(false)
    ...
    override fun authenticate(
        ...
        if (authInProgress.get()) {
            completeWithValue<AuthResult>(
                callback, AuthResult(AuthResultCode.ALREADY_IN_PROGRESS, null)
            )
            return
        }
        ...
        if (!isDeviceSupported()) {
            completeWithValue<AuthResult>(
                callback, AuthResult(AuthResultCode.NO_CREDENTIALS, null)
            )
            return
        }
