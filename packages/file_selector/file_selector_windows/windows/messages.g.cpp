...
void TypeGroup::set_extensions(const EncodableList& value_arg) { extensions_ = value_arg; }
...
TypeGroup::TypeGroup(const EncodableList& list) {
  ...
  if (const std::string* pointer_label = std::get_if<std::string>(&encodable_label)) {
    label_ = *pointer_label;
  }
  auto& encodable_extensions = list[1];
  if (const EncodableList* pointer_extensions = std::get_if<EncodableList>(&encodable_extensions)) {
...
void SelectionOptions::set_allow_multiple(bool value_arg) { allow_multiple_ = value_arg; }

bool SelectionOptions::select_folders() const { return select_folders_; }
void SelectionOptions::set_select_folders(bool value_arg) { select_folders_ = value_arg; }

const EncodableList& SelectionOptions::allowed_types() const { return allowed_types_; }
void SelectionOptions::set_allowed_types(const EncodableList& value_arg) { allowed_types_ = value_arg; }
...
SelectionOptions::SelectionOptions(const EncodableList& list) {
  ...
  if (const bool* pointer_allow_multiple = std::get_if<bool>(&encodable_allow_multiple)) {
    allow_multiple_ = *pointer_allow_multiple;
  }
  auto& encodable_select_folders = list[1];
  if (const bool* pointer_select_folders = std::get_if<bool>(&encodable_select_folders)) {
    select_folders_ = *pointer_select_folders;
  }
  auto& encodable_allowed_types = list[2];
  if (const EncodableList* pointer_allowed_types = std::get_if<EncodableList>(&encodable_allowed_types)) {
    allowed_types_ = *pointer_allowed_types;
  }
}

FileSelectorApiCodecSerializer::FileSelectorApiCodecSerializer() {}
EncodableValue FileSelectorApiCodecSerializer::ReadValueOfType(uint8_t type, flutter::ByteStreamReader* stream) const {
  switch (type) {
    case 128:
      return CustomEncodableValue(SelectionOptions(std::get<EncodableList>(ReadValue(stream))));
    case 129:
      return CustomEncodableValue(TypeGroup(std::get<EncodableList>(ReadValue(stream))));
    default:
      return flutter::StandardCodecSerializer::ReadValueOfType(type, stream);
  }
}

void FileSelectorApiCodecSerializer::WriteValue(const EncodableValue& value, flutter::ByteStreamWriter* stream) const {
...
const flutter::StandardMessageCodec& FileSelectorApi::GetCodec() {
  return flutter::StandardMessageCodec::GetInstance(&FileSelectorApiCodecSerializer::GetInstance());
}

// Sets up an instance of `FileSelectorApi` to handle messages through the `binary_messenger`.
void FileSelectorApi::SetUp(flutter::BinaryMessenger* binary_messenger, FileSelectorApi* api) {
  {
    auto channel = std::make_unique<BasicMessageChannel<>>(binary_messenger, "dev.flutter.pigeon.FileSelectorApi.showOpenDialog", &GetCodec());
    if (api != nullptr) {
      channel->SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          ErrorOr<EncodableList> output = api->ShowOpenDialog(options_arg, initial_directory_arg, confirm_button_text_arg);
          if (output.has_error()) {
  ...
  {
    auto channel = std::make_unique<BasicMessageChannel<>>(binary_messenger, "dev.flutter.pigeon.FileSelectorApi.showSaveDialog", &GetCodec());
    if (api != nullptr) {
      channel->SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          ...
          const auto* confirm_button_text_arg = std::get_if<std::string>(&encodable_confirm_button_text_arg);
          ErrorOr<EncodableList> output = api->ShowSaveDialog(options_arg, initial_directory_arg, suggested_name_arg, confirm_button_text_arg);
          if (output.has_error()) {
            reply(WrapError(output.error()));
            return;
          }
