...
bool MessageData::operator==(const MessageData& other) const {
  return PigeonInternalDeepEquals(name_, other.name_) && PigeonInternalDeepEquals(description_, other.description_) && PigeonInternalDeepEquals(code_, other.code_) && PigeonInternalDeepEquals(data_, other.data_);
}
...
size_t PigeonInternalDeepHash(const MessageData& v) {
  return v.Hash();
}
...
void ExampleHostApi::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.pigeon_example_package.ExampleHostApi.sendMessage" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const ::flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
...
void MessageFlutterApi::FlutterMethod(
  ...
  const std::string channel_name = "dev.flutter.pigeon.pigeon_example_package.MessageFlutterApi.flutterMethod" + message_channel_suffix_;
