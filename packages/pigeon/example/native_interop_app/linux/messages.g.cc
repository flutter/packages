...
struct _PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse {
  GObject parent_instance;
...
G_DEFINE_TYPE(
    PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse,
    pigeon_example_package_example_host_api_determine_host_language_response,
    G_TYPE_OBJECT)

static void
pigeon_example_package_example_host_api_determine_host_language_response_dispose(
    GObject* object) {
  PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse* self =
      PIGEON_EXAMPLE_PACKAGE_EXAMPLE_HOST_API_DETERMINE_HOST_LANGUAGE_RESPONSE(
          object);
  g_clear_pointer(&self->value, fl_value_unref);
  G_OBJECT_CLASS(
      pigeon_example_package_example_host_api_determine_host_language_response_parent_class)
      ->dispose(object);
}

static void
pigeon_example_package_example_host_api_determine_host_language_response_init(
    PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse* self) {}

static void
pigeon_example_package_example_host_api_determine_host_language_response_class_init(
    PigeonExamplePackageExampleHostApiDetermineHostLanguageResponseClass*
        klass) {
  G_OBJECT_CLASS(klass)->dispose =
      pigeon_example_package_example_host_api_determine_host_language_response_dispose;
}
...
PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse*
pigeon_example_package_example_host_api_determine_host_language_response_new(
    const gchar* return_value) {
  PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse* self =
      PIGEON_EXAMPLE_PACKAGE_EXAMPLE_HOST_API_DETERMINE_HOST_LANGUAGE_RESPONSE(
          g_object_new(
              pigeon_example_package_example_host_api_determine_host_language_response_get_type(),
              nullptr));
...
PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse*
pigeon_example_package_example_host_api_determine_host_language_response_new_error(
...
static void pigeon_example_package_example_host_api_determine_host_language_cb(
  ...
  if (self->vtable == nullptr ||
      self->vtable->determine_host_language == nullptr) {
    return;
  }

  g_autoptr(PigeonExamplePackageExampleHostApiDetermineHostLanguageResponse)
      response = self->vtable->determine_host_language(self->user_data);
  if (response == nullptr) {
  ...
  if (!fl_basic_message_channel_respond(channel, response_handle,
    ...
    g_warning("Failed to send response to %s.%s: %s", "ExampleHostApi",
              "determineHostLanguage", error->message);
  }
}
...
void pigeon_example_package_example_host_api_set_method_handlers(
  ...
  g_autoptr(PigeonExamplePackageMessageCodec) codec =
      pigeon_example_package_message_codec_new();
  g_autofree gchar* determine_host_language_channel_name = g_strdup_printf(
      "dev.flutter.pigeon.pigeon_example_package.ExampleHostApi."
      "determineHostLanguage%s",
      dot_suffix);
  g_autoptr(FlBasicMessageChannel) determine_host_language_channel =
      fl_basic_message_channel_new(messenger,
                                   determine_host_language_channel_name,
                                   FL_MESSAGE_CODEC(codec));
  fl_basic_message_channel_set_message_handler(
      determine_host_language_channel,
      pigeon_example_package_example_host_api_determine_host_language_cb,
...
void pigeon_example_package_example_host_api_clear_method_handlers(
  ...
  g_autoptr(FlBasicMessageChannel) determine_host_language_channel =
      fl_basic_message_channel_new(messenger,
                                   determine_host_language_channel_name,
                                   FL_MESSAGE_CODEC(codec));
  fl_basic_message_channel_set_message_handler(determine_host_language_channel,
