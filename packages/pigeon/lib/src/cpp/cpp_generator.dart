...
final Class _overflowClass = Class(
  name: _overflowClassName,
  fields: _overflowFields,
);
    ...
    indent.addScoped('{', '};', () {
      _writeAccessBlock(indent, _ClassAccess.public, () {
        ...
        if (orderedFields.any(
          (NamedType field) => _isPointerField(
  ...
  void writeClassDecode(
    ...
    _writeFunctionDefinition(
      ...
      body: () {
        ...
        indent.format(
          '${classDefinition.name} $instanceVariable$constructorArgs;',
        );
...
EncodableValue $_overflowClassName::FromEncodableList(
  ...
  return $_overflowClassName(list[0].LongValue(),
    ...
    indent.writeScoped(
      'EncodableValue $_overflowClassName::Unwrap() {',
      '}',
      () {
          ...
          indent.addScoped('{', '}', () {
            void writeEncodeLogic(EnumeratedType customType) {
              ...
              indent.addScoped('{', '}', () {
                ...
                indent.writeln(
                  'WriteValue(EncodableValue($valueString), stream);',
                );
  ...
  void writeFlutterApi(
    ...
    for (final Method func in api.methods) {
      ...
      final Iterable<_HostNamedType> hostParameters = indexMap(
        func.parameters,
        (int i, NamedType arg) {
          final HostDatatype hostType = getFieldHostDatatype(
            arg,
          ...
          if (func.parameters.isEmpty) {
            ...
            indent.addScoped('EncodableValue(EncodableList{', '});', () {
              for (final param in hostParameters) {
                final String encodedArgument =
                    _wrappedHostApiArgumentExpression(
                      root,
                      param.name,
                      param.originalType,
                      param.hostType,
...
\terror.details()
  ...
  void _writeHostMethodMessageHandler(
    ...
    indent.writeScoped('{', '}', () {
      ...
      _writeMessageHandlerRegistration(
        indent,
        varChannelName: varChannelName,
        setHandlerCondition: 'api != nullptr',
  ...
  void _writeChannelAllocation(
    ...
    indent.writeln(
      'BasicMessageChannel<> $varChannelName(binary_messenger, '
  ...
  void _writeMessageHandlerRegistration(
    ...
    indent.writeScoped('if ($setHandlerCondition) {', '} else {', () {
      indent.write(
        '$varChannelName.SetMessageHandler([api](const EncodableValue& $messageVarName, const ::flutter::MessageReply<EncodableValue>& reply) ',
      );
      indent.addScoped('{', '});', () {
    ...
    indent.addScoped(null, '}', () {
      indent.writeln('$varChannelName.SetMessageHandler(nullptr);');
    });
  }
