...
void main() {
  test('gen one class', () {
    ...
    expect(code, contains('fun fromList(pigeon_list: List<Any?>): Foobar'));
  ...
  test('gen class with enum', () {
    ...
    expect(code, contains('fun fromList(pigeon_list: List<Any?>): Bar'));
    expect(code, contains('Foo.ofRaw(it)'));
    expect(code, contains('val field1 = pigeon_list[0] as Foo'));
    expect(code, contains('val field2 = pigeon_list[1] as String\n'));
    expect(code, contains('fun toList(): List<Any?>'));
  });
  ...
  test('all the simple datatypes header', () {
    ...
    expect(
        code,
        contains(
            'val aInt = pigeon_list[1].let { num -> if (num is Int) num.toLong() else num as Long }'));
    ...
    expect(
        code,
        contains(
            'val aNullableInt = pigeon_list[9].let { num -> if (num is Int) num.toLong() else num as Long? }'));
  });
  ...
  test('gen nested', () {
    ...
    expect(code, contains('fun fromList(pigeon_list: List<Any?>): Outer'));
    expect(code, contains('val nested = pigeon_list[0] as Nested?'));
    expect(code, contains('fun toList(): List<Any?>'));
  });
