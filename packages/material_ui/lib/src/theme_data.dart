...
part 'generated/theme_data_defaults.g.dart';
...
/// A Material 3 color scheme contrast level.
///
/// The named constants [standard], [medium], and [high] select the token-backed
/// default Material 3 color schemes. Use [ContrastLevel.new] to provide a
/// custom value between -1.0 and 1.0 when generating a color scheme from
/// [ThemeData.colorSchemeSeed].
@immutable
class ContrastLevel {
  /// Creates a custom contrast level.
  const ContrastLevel(this.value) : assert(value >= -1.0 && value <= 1.0);

  /// Standard contrast, corresponding to a contrast level of 0.0.
  static const ContrastLevel standard = ContrastLevel(0.0);
  ...
  /// The numeric contrast level passed to color scheme generation.
  final double value;
  ...
  @override
  String toString() => 'ContrastLevel($value)';
}
...
class ThemeData with Diagnosticable {
  ...
  /// schemes. Values must be between -1.0 and 1.0 inclusive. Use
  /// [ContrastLevel.standard], [ContrastLevel.medium], or [ContrastLevel.high]
  /// for the token-backed default Material 3 contrast levels. Other values are
  /// applied when generating a color scheme from [colorSchemeSeed].
  ///
...
ColorScheme _defaultM3ColorScheme(Brightness brightness, ContrastLevel contrastLevel) {
