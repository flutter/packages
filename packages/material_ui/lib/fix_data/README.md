...
used by the material_ui package.
...
file is not for generic library fixes (`fix_material.yaml`), ensure it is named
...
See the packages/material_ui/test_fixes directory for the tests that
validate these fix rules.

To run these tests locally, execute this command in the
packages/material_ui/test_fixes directory.
```sh
