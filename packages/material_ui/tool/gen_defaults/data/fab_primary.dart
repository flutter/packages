// Copyright 2013 The Flutter Authors.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Version: 38.0.1

import 'color_role.dart';

class TokenFabPrimary {
  /// md.comp.fab.primary.container.elevation
  static const double containerElevation = 6.00;

  /// md.comp.fab.primary.focused.container.elevation
  static const double focusedContainerElevation = 6.00;

  /// md.comp.fab.primary.focused.state-layer.opacity
  static const double focusedStateLayerOpacity = 0.10;

  /// md.comp.fab.primary.hovered.container.elevation
  static const double hoveredContainerElevation = 8.00;

  /// md.comp.fab.primary.hovered.state-layer.opacity
  static const double hoveredStateLayerOpacity = 0.08;

  /// md.comp.fab.primary.pressed.container.elevation
  static const double pressedContainerElevation = 6.00;

  /// md.comp.fab.primary.pressed.state-layer.opacity
  static const double pressedStateLayerOpacity = 0.10;
}

class TokenFabPrimaryDark {
  /// md.comp.fab.primary.container.shadow-color
  static const TokenColorRole containerShadowColor = TokenColorRole.shadow;
}

class TokenFabPrimaryDarkDefault {
  /// md.comp.fab.primary.container.color
  static const TokenColorRole containerColor = TokenColorRole.primary;

  /// md.comp.fab.primary.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onPrimary;
}

class TokenFabPrimaryDarkHighContrast {
  /// md.comp.fab.primary.container.color
  static const TokenColorRole containerColor = TokenColorRole.primary;

  /// md.comp.fab.primary.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onPrimary;
}

class TokenFabPrimaryDarkMediumContrast {
  /// md.comp.fab.primary.container.color
  static const TokenColorRole containerColor = TokenColorRole.primary;

  /// md.comp.fab.primary.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onPrimary;
}

class TokenFabPrimaryLight {
  /// md.comp.fab.primary.container.shadow-color
  static const TokenColorRole containerShadowColor = TokenColorRole.shadow;
}

class TokenFabPrimaryLightDefault {
  /// md.comp.fab.primary.container.color
  static const TokenColorRole containerColor = TokenColorRole.primary;

  /// md.comp.fab.primary.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onPrimary;
}

class TokenFabPrimaryLightHighContrast {
  /// md.comp.fab.primary.container.color
  static const TokenColorRole containerColor = TokenColorRole.primary;

  /// md.comp.fab.primary.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onPrimary;
}

class TokenFabPrimaryLightMediumContrast {
  /// md.comp.fab.primary.container.color
  static const TokenColorRole containerColor = TokenColorRole.primary;

  /// md.comp.fab.primary.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onPrimary;

  /// md.comp.fab.primary.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onPrimary;
}
