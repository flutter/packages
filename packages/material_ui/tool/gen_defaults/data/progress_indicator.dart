// Copyright 2013 The Flutter Authors.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Version: 38.0.1

import 'color_role.dart';
import 'shape_struct.dart';

class TokenProgressIndicator {
  /// md.comp.progress-indicator.active-indicator.shape
  static const ShapeStruct activeIndicatorShape = ShapeStruct(
    family: 'SHAPE_FAMILY_CIRCULAR',
    topLeft: 0.00,
    topRight: 0.00,
    bottomLeft: 0.00,
    bottomRight: 0.00,
  );

  /// md.comp.progress-indicator.stop-indicator.shape
  static const ShapeStruct stopIndicatorShape = ShapeStruct(
    family: 'SHAPE_FAMILY_CIRCULAR',
    topLeft: 0.00,
    topRight: 0.00,
    bottomLeft: 0.00,
    bottomRight: 0.00,
  );

  /// md.comp.progress-indicator.track.shape
  static const ShapeStruct trackShape = ShapeStruct(
    family: 'SHAPE_FAMILY_CIRCULAR',
    topLeft: 0.00,
    topRight: 0.00,
    bottomLeft: 0.00,
    bottomRight: 0.00,
  );
}

class TokenProgressIndicatorDarkDefault {
  /// md.comp.progress-indicator.active-indicator.color
  static const TokenColorRole activeIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.stop-indicator.color
  static const TokenColorRole stopIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.track.color
  static const TokenColorRole trackColor = TokenColorRole.secondaryContainer;
}

class TokenProgressIndicatorDarkHighContrast {
  /// md.comp.progress-indicator.active-indicator.color
  static const TokenColorRole activeIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.stop-indicator.color
  static const TokenColorRole stopIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.track.color
  static const TokenColorRole trackColor = TokenColorRole.secondaryContainer;
}

class TokenProgressIndicatorDarkMediumContrast {
  /// md.comp.progress-indicator.active-indicator.color
  static const TokenColorRole activeIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.stop-indicator.color
  static const TokenColorRole stopIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.track.color
  static const TokenColorRole trackColor = TokenColorRole.secondaryContainer;
}

class TokenProgressIndicatorLightDefault {
  /// md.comp.progress-indicator.active-indicator.color
  static const TokenColorRole activeIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.stop-indicator.color
  static const TokenColorRole stopIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.track.color
  static const TokenColorRole trackColor = TokenColorRole.secondaryContainer;
}

class TokenProgressIndicatorLightHighContrast {
  /// md.comp.progress-indicator.active-indicator.color
  static const TokenColorRole activeIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.stop-indicator.color
  static const TokenColorRole stopIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.track.color
  static const TokenColorRole trackColor = TokenColorRole.secondaryContainer;
}

class TokenProgressIndicatorLightMediumContrast {
  /// md.comp.progress-indicator.active-indicator.color
  static const TokenColorRole activeIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.stop-indicator.color
  static const TokenColorRole stopIndicatorColor = TokenColorRole.primary;

  /// md.comp.progress-indicator.track.color
  static const TokenColorRole trackColor = TokenColorRole.secondaryContainer;
}
