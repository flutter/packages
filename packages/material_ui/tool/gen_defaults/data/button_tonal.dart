// Copyright 2013 The Flutter Authors.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Version: 36.0.6

import 'color_role.dart';

class TokenButtonTonal {
  /// md.comp.button.tonal.container.color
  static const TokenColorRole containerColor = TokenColorRole.secondaryContainer;

  /// md.comp.button.tonal.container.elevation
  static const double containerElevation = 0.00;

  /// md.comp.button.tonal.container.shadow-color
  static const TokenColorRole containerShadowColor = TokenColorRole.shadow;

  /// md.comp.button.tonal.disabled.container.color
  static const TokenColorRole disabledContainerColor = TokenColorRole.onSurface;

  /// md.comp.button.tonal.disabled.container.elevation
  static const double disabledContainerElevation = 0.00;

  /// md.comp.button.tonal.disabled.container.opacity
  static const double disabledContainerOpacity = 0.10;

  /// md.comp.button.tonal.disabled.icon.color
  static const TokenColorRole disabledIconColor = TokenColorRole.onSurface;

  /// md.comp.button.tonal.disabled.icon.opacity
  static const double disabledIconOpacity = 0.38;

  /// md.comp.button.tonal.disabled.label-text.color
  static const TokenColorRole disabledLabelTextColor = TokenColorRole.onSurface;

  /// md.comp.button.tonal.disabled.label-text.opacity
  static const double disabledLabelTextOpacity = 0.38;

  /// md.comp.button.tonal.focused.container.elevation
  static const double focusedContainerElevation = 0.00;

  /// md.comp.button.tonal.focused.icon.color
  static const TokenColorRole focusedIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.focused.label-text.color
  static const TokenColorRole focusedLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.focused.state-layer.color
  static const TokenColorRole focusedStateLayerColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.focused.state-layer.opacity
  static const double focusedStateLayerOpacity = 0.10;

  /// md.comp.button.tonal.hovered.icon.color
  static const TokenColorRole hoveredIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.hovered.label-text.color
  static const TokenColorRole hoveredLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.hovered.state-layer.color
  static const TokenColorRole hoveredStateLayerColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.hovered.state-layer.opacity
  static const double hoveredStateLayerOpacity = 0.08;

  /// md.comp.button.tonal.icon.color
  static const TokenColorRole iconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.label-text.color
  static const TokenColorRole labelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.pressed.container.elevation
  static const double pressedContainerElevation = 0.00;

  /// md.comp.button.tonal.pressed.icon.color
  static const TokenColorRole pressedIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.pressed.label-text.color
  static const TokenColorRole pressedLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.pressed.state-layer.color
  static const TokenColorRole pressedStateLayerColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.pressed.state-layer.opacity
  static const double pressedStateLayerOpacity = 0.10;

  /// md.comp.button.tonal.selected.container.color
  static const TokenColorRole selectedContainerColor = TokenColorRole.secondary;

  /// md.comp.button.tonal.selected.focused.icon.color
  static const TokenColorRole selectedFocusedIconColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.focused.label-text.color
  static const TokenColorRole selectedFocusedLabelTextColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.focused.state-layer.color
  static const TokenColorRole selectedFocusedStateLayerColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.hovered.icon.color
  static const TokenColorRole selectedHoveredIconColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.hovered.label-text.color
  static const TokenColorRole selectedHoveredLabelTextColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.hovered.state-layer.color
  static const TokenColorRole selectedHoveredStateLayerColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.icon.color
  static const TokenColorRole selectedIconColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.label-text.color
  static const TokenColorRole selectedLabelTextColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.pressed.icon.color
  static const TokenColorRole selectedPressedIconColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.pressed.label-text.color
  static const TokenColorRole selectedPressedLabelTextColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.selected.pressed.state-layer.color
  static const TokenColorRole selectedPressedStateLayerColor = TokenColorRole.onSecondary;

  /// md.comp.button.tonal.unselected.container.color
  static const TokenColorRole unselectedContainerColor = TokenColorRole.secondaryContainer;

  /// md.comp.button.tonal.unselected.focused.icon.color
  static const TokenColorRole unselectedFocusedIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.focused.label-text.color
  static const TokenColorRole unselectedFocusedLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.focused.state-layer.color
  static const TokenColorRole unselectedFocusedStateLayerColor =
      TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.hovered.icon.color
  static const TokenColorRole unselectedHoveredIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.hovered.label-text.color
  static const TokenColorRole unselectedHoveredLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.hovered.state-layer.color
  static const TokenColorRole unselectedHoveredStateLayerColor =
      TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.icon.color
  static const TokenColorRole unselectedIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.label-text.color
  static const TokenColorRole unselectedLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.pressed.icon.color
  static const TokenColorRole unselectedPressedIconColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.pressed.label-text.color
  static const TokenColorRole unselectedPressedLabelTextColor = TokenColorRole.onSecondaryContainer;

  /// md.comp.button.tonal.unselected.pressed.state-layer.color
  static const TokenColorRole unselectedPressedStateLayerColor =
      TokenColorRole.onSecondaryContainer;
}
