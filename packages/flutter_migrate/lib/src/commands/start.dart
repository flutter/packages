...
class MigrateStartCommand extends MigrateCommand {
  ...
  Future<CommandResult> runCommand() async {
    ...
    final String? projectRootDirPath = stringArg('project-directory') ?? environment.getString('FlutterProject.directory');
    ...
    final FlutterProject project = projectRootDirPath == null
      ...
      : flutterProjectFactory.fromDirectory(fileSystem.directory(projectRootDirPath));

    final bool isModule = environment.getBool('FlutterProject.isModule') ?? false;
    final bool isPlugin = environment.getBool('FlutterProject.isPlugin') ?? false;
    if (isModule || isPlugin) {
      logger.printError('Migrate tool only supports app projects. This project is a ${isModule ? 'module' : 'plugin'}');
      return const CommandResult(ExitStatus.fail);
    }
    ...
    await writeStagingDir(migrateResult, logger, verbose: _verbose, projectRootDir: projectRootDir);
  ...
  bool _validateEnvironment(FlutterToolsEnvironment environment) {
    if (environment.getString('FlutterProject.directory') == null) {
      logger.printError('No valid flutter project found. This command must be run from a flutter project directory');
      return false;
    }
    ...
    if (!(environment.getBool('FlutterProject.android.exists') ?? false) && environment['FlutterProject.android.isKotlin'] == null) {
      logger.printError('Could not detect if android project uses kotlin or java');
      return false;
    }
    if (!(environment.getBool('FlutterProject.ios.exists') ?? false) && environment['FlutterProject.ios.isSwift'] == null) {
      logger.printError('Could not detect if iosProject uses swift or objective-c');
      return false;
    }
    return true;
  }
  ...
  Future<void> writeStagingDir(MigrateResult migrateResult, Logger logger, {bool verbose = false, required Directory projectRootDir}) async {
    final Directory stagingDir = projectRootDir.childDirectory(kDefaultMigrateStagingDirectoryName);
    if (verbose) {
