...
Future<MigrateResult?> computeMigration({
  ...
  final String androidLanguage = environment['FlutterProject.android.isKotlin']! as bool ? 'kotlin' : 'java';
  final String iosLanguage = environment['FlutterProject.ios.isSwift']! as bool ? 'swift' : 'objc';

  final Directory targetFlutterDirectory = fileSystem.directory(environment.getString('Cache.flutterRoot'));
...
abstract class MigrateFlutterProject {
  ...
  Future<List<FilePendingMigration>> newlyAddedFiles(MigrateContext context, MigrateFlutterProject other) async {
    ...
    if (context.verbose) {
      context.status.pause();
      context.logger.printStatus('${addedFiles.length} files were newly added in the target app.');
      context.status.resume();
    }
    return addedFiles;
  }
  ...
  static Future<void> merge(
    ...
    for (final FileSystemEntity entity in currentFiles) {
      ...
      if (userDiff.exitCode == 0 || alwaysMigrate) {
        if ((context.migrateResult.diffMap.containsKey(localPath) || alwaysMigrate) && context.migrateResult.diffMap[localPath] != null ) {
...
class MigrateRevisions {
  MigrateRevisions({
    ...
    _computeRevisions(context, baseRevision, allowFallbackBaseRevision, platforms, environment);
  }
