...
class MigrateUtils {
  ...
  Future<ProcessResult> _runCommand(List<String> command, {String? workingDirectory}) {
  ...
  Future<String> createFromTemplates(String flutterBinPath, {
    ...
    final ProcessResult result = await _runCommand(cmdArgs, workingDirectory: outputDirectory);
  ...
  Future<void> gitInit(String workingDirectory) async {
    ...
    final ProcessResult result = await _runCommand(cmdArgs, workingDirectory: workingDirectory);
    checkForErrors(result, commandDescription: cmdArgs.join(' '));
  }
...
Future<bool> hasUncommittedChanges(String projectDirectory, Logger logger, MigrateUtils migrateUtils) async {
  if (await migrateUtils.hasUncommittedChanges(projectDirectory)) {
    ...
    logger.printStatus('You may commit your changes using \'git add .\' and \'git commit -m "<message"\'');
    return false;
  }
  return false;
}
...
void printCommandText(String command, Logger logger, {bool? standalone = true}) {
  final String prefix = standalone == null ? '' : (standalone ? './bin/flutter_migrate ' : 'flutter migrate ');
  logger.printStatus(
    '\n\$ $prefix$command\n',
