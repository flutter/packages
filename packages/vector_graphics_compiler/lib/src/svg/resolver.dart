...
class ResolvingVisitor extends Visitor<Node, AffineMatrix> {
  ...
  Node visitTextNode(TextNode textNode, AffineMatrix data) {
    ...
    if (paint == null || textConfig.text.trim().isEmpty) {
      return Node.empty;
    }
    if (_currentLayer != null &&
        (_currentLayer!.isSourceAlpha || _currentLayer!.sigmaX > 0 || _currentLayer!.sigmaY > 0)) {
      return ResolvedTextNode(
        ...
        paint: _createLayerPaint(paint, _currentLayer!, data),
      );
    }
  ...
  Gradient _blackenGradient(Gradient gradient) {
    ...
    final List<Color> colors = gradient.colors!
        .map((Color c) => Color.fromARGB(c.a, 0, 0, 0))
        .toList();
  ...
  Paint _createLayerPaint(
    Paint originalPaint,
    SvgFilterLayer layer, [
    AffineMatrix transform = AffineMatrix.identity,
  ]) {
    final bool hasBlur = layer.sigmaX > 0 || layer.sigmaY > 0;
