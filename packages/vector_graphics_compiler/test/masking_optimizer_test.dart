...
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <g mask="url(#a)">
    ...
    final List<ResolvedMaskNode> maskNodesNew = queryChildren<ResolvedMaskNode>(newNode);
...
''');
  ...
  test("Don't resolve a MaskNode if one of PathNodes it's applied to has stroke.width set", () {
    final Node node = parseAndResolve('''
...
''');
  ...
  test('Masks on groups', () {
    final VectorInstructions instructions = parse(groupMask, enableMaskingOptimizer: false);
    ...
    expect(instructions.paths, <Path>[
      ...
      Path(
        commands: const <PathCommand>[
          ...
          CubicToCommand(279.0881677156519, 0.0, 288.75, 9.661832284348126, 288.75, 21.5625),
          LineToCommand(288.75, 506.4375),
          CubicToCommand(288.75, 518.3381677156518, 279.0881677156519, 528.0, 267.1875, 528.0),
          LineToCommand(21.5625, 528.0),
          CubicToCommand(9.661832284348126, 528.0, 0.0, 518.3381677156518, 0.0, 506.4375),
          LineToCommand(0.0, 21.5625),
          CubicToCommand(0.0, 9.661832284348126, 9.661832284348126, 0.0, 21.5625, 0.0),
          CloseCommand(),
        ],
      ),
    ]);
