...
''');
  ...
  test('Use handles stroke and fill correctly', () {
    final VectorInstructions instructions = parseWithoutOptimizers(useStar);
  ...
  test('Use preserves fill from shape', () {
    final VectorInstructions instructions = parseWithoutOptimizers(useColor);
  ...
  test('Image in defs', () {
    final VectorInstructions instructions = parseWithoutOptimizers(imageInDefs);
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="main-svg" width="100" height="100" viewBox="0 0 100 100">
  ...
  test('text anchor', () {
    final VectorInstructions instructions = parseWithoutOptimizers(textAnchors);
  ...
  test('Clip with use', () {
    final VectorInstructions instructions = parseWithoutOptimizers(basicClip);
    final VectorInstructions instructions2 = parseWithoutOptimizers(useClip);
...
<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  ...
  test('focal radial', () {
    final VectorInstructions instructions = parseWithoutOptimizers(focalRadial);
...
''';
  ...
  test('Handles masks correctly', () {
    final VectorInstructions instructions = parseWithoutOptimizers(basicMask);
    expect(
      instructions.paths,
      <Path>[
...
''';
    ...
    expect(
      instructions.commands.where((DrawCommand c) => c.type == DrawCommandType.text).length,
      3,
    );
  });
...
''';
  ...
  test('Preserves radial gradient stop opacity and stroke on SourceAlpha filter', () {
    const svg = '''
<svg viewBox="0 0 100 100">
  <defs>
  ...
  <rect width="100" height="100" fill="url(#rgrad)" stroke="url(#rgrad)" stroke-width="2" filter="url(#shadow)"/>
</svg>
''';
    final VectorInstructions instructions = parseWithoutOptimizers(svg);
    expect(instructions.paints.length, 1);
    final Paint paint = instructions.paints.first;
    expect(paint.fill?.shader?.colors, const <Color>[Color.opaqueBlack, Color(0x7F000000)]);
    expect(paint.stroke?.shader?.colors, const <Color>[Color.opaqueBlack, Color(0x7F000000)]);
    expect(paint.stroke?.width, 2.0);
  });

  test('Handles feMergeNode referencing missing input', () {
    const svg = '''
<svg viewBox="0 0 100 100">
  <filter id="broken-merge">
    <feMerge>
      <feMergeNode in="nonexistent" />
    </feMerge>
  </filter>
  <rect width="100" height="100" fill="blue" filter="url(#broken-merge)"/>
</svg>
''';
    final VectorInstructions instructions = parseWithoutOptimizers(svg);
    expect(instructions.paints, const <Paint>[Paint(fill: Fill(color: Color(0xff0000ff)))]);
  ...
  test('SvgFilter and SvgFilterLayer equality and hashCode', () {
    const layer1 = SvgFilterLayer(isSourceAlpha: true, sigmaX: 2, sigmaY: 3, dx: 4, dy: 5);
    // ignore: prefer_const_constructors
    final layer2 = SvgFilterLayer(isSourceAlpha: true, sigmaX: 2, sigmaY: 3, dx: 4, dy: 5);
    const layer3 = SvgFilterLayer(isSourceAlpha: false, sigmaX: 0, sigmaY: 0, dx: 0, dy: 0);
    expect(layer1, equals(layer2));
    expect(layer1.hashCode, equals(layer2.hashCode));
    expect(layer1, isNot(equals(layer3)));
    expect(layer1.hasEffect, isTrue);
    expect(layer3.hasEffect, isFalse);

    const filter1 = SvgFilter(<SvgFilterLayer>[layer1, layer3]);
    final filter2 = SvgFilter(<SvgFilterLayer>[layer2, layer3]);
    const filter3 = SvgFilter(<SvgFilterLayer>[layer3]);
    expect(filter1, equals(filter2));
    expect(filter1.hashCode, equals(filter2.hashCode));
    expect(filter1, isNot(equals(filter3)));
  });

  test('Supports filters on <use> and referenced elements', () {
    const svg = '''
<svg viewBox="0 0 100 100">
  <defs>
    ...
    <rect id="r2" width="20" height="20" fill="blue" filter="url(#blur2)"/>
    <g id="g1"><circle cx="10" cy="10" r="10" fill="green"/></g>
    <text id="t1" x="10" y="20" fill="black">Hi</text>
    <use id="u1" href="#r1"/>
  </defs>
  <use href="#r1" x="5" y="5" filter="url(#blur1)"/>
  <use href="#r2" x="30" y="5"/>
  <use href="#g1" x="55" y="5" filter="url(#blur1)"/>
  <use href="#t1" x="5" y="40" filter="url(#blur2)"/>
  <use href="#u1" x="50" y="40" filter="url(#blur1)"/>
</svg>
''';
    final VectorInstructions instructions = parseWithoutOptimizers(svg);
    expect(
      instructions.paints.where((Paint p) => p.filterBlurX == 3.0 && p.filterBlurY == 3.0),
      hasLength(3),
    );
    expect(
      instructions.paints.where((Paint p) => p.filterBlurX == 5.0 && p.filterBlurY == 5.0),
      hasLength(2),
    );
  });
