...
@implementation FLTApiVideoQualityBox
- (instancetype)initWithValue:(FLTApiVideoQuality)value {
  self = [super init];
  if (self) {
    _value = value;
  }
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 131: {
      NSNumber *enumAsNumber = [self readValue];
      return enumAsNumber == nil ? nil : [[FLTApiVideoQualityBox alloc] initWithValue:[enumAsNumber integerValue]];
    }
    case 132: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FLTSourceCameraBox class]]) {
    ...
    [self writeByte:130];
    [self writeValue:(value == nil ? [NSNull null] : [NSNumber numberWithInteger:box.value])];
  } else if ([value isKindOfClass:[FLTApiVideoQualityBox class]]) {
    FLTApiVideoQualityBox *box = (FLTApiVideoQualityBox *)value;
    [self writeByte:131];
    [self writeValue:(value == nil ? [NSNull null] : [NSNumber numberWithInteger:box.value])];
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FLTGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(pickVideoWithSource:maxDuration:videoQuality:completion:)], @"FLTImagePickerApi api (%@) doesn't respond to @selector(pickVideoWithSource:maxDuration:videoQuality:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        FLTSourceSpecification *arg_source = GetNullableObjectAtIndex(args, 0);
        NSNumber *arg_maxDurationSeconds = GetNullableObjectAtIndex(args, 1);
        FLTApiVideoQualityBox *arg_videoQuality = GetNullableObjectAtIndex(args, 2);
        [api pickVideoWithSource:arg_source maxDuration:arg_maxDurationSeconds videoQuality:arg_videoQuality completion:^(NSString *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
