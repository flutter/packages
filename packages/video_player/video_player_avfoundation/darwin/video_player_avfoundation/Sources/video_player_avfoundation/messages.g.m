  ...
  }
  {
    FlutterBasicMessageChannel *channel = [[FlutterBasicMessageChannel alloc]
           initWithName:[NSString
                            stringWithFormat:@"%@%@",
                                             @"dev.flutter.pigeon.video_player_avfoundation."
                                             @"AVFoundationVideoPlayerApi.seekToDefaultPosition",
                                             messageChannelSuffix]
        binaryMessenger:binaryMessenger
                  codec:FVPGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(seekToDefaultPositionForPlayer:completion:)],
                @"FVPAVFoundationVideoPlayerApi api (%@) doesn't respond to "
                @"@selector(seekToDefaultPositionForPlayer:completion:)",
                api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSInteger arg_playerId = [GetNullableObjectAtIndex(args, 0) integerValue];
        [api seekToDefaultPositionForPlayer:arg_playerId
                                 completion:^(FlutterError *_Nullable error) {
                                   callback(wrapResult(nil, error));
                                 }];
      }];
