...
+ (FVPAudioTrackMessage *)fromList:(NSArray *)list {
  ...
  pigeonResult.groupId = [GetNullableObjectAtIndex(list, 1) integerValue];
  pigeonResult.trackId = [GetNullableObjectAtIndex(list, 2) integerValue];
  return pigeonResult;
}
+ (nullable FVPAudioTrackMessage *)nullableFromList:(NSArray *)list {
...
void SetUpFVPAVFoundationVideoPlayerApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FVPAVFoundationVideoPlayerApi> *api, NSString *messageChannelSuffix) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.video_player_avfoundation.AVFoundationVideoPlayerApi.changeAudioTrack", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FVPAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(changeAudioTrack:error:)], @"FVPAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(changeAudioTrack:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FVPAudioTrackMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api changeAudioTrack:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
