...
- (void)removeRequest:(AVAssetResourceLoadingRequest *)request {
    ...
    [self.pendingRequestWorkers enumerateObjectsUsingBlock:^(ResourceLoadingRequestWorker *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)startNoCacheWorkerWithRequest:(AVAssetResourceLoadingRequest *)request {
    ...
    ContentDownloader *contentDownloader = [[ContentDownloader alloc] initWithURL:self.url cacheWorker:self.cacheWorker];
    ResourceLoadingRequestWorker *requestWorker = [[ResourceLoadingRequestWorker alloc] initWithContentDownloader:contentDownloader
...
- (void)startWorkerWithRequest:(AVAssetResourceLoadingRequest *)request {
    [[ContentDownloaderStatus shared] addURL:self.url];
    ResourceLoadingRequestWorker *requestWorker = [[ResourceLoadingRequestWorker alloc] initWithContentDownloader:self.contentDownloader
                                                                                             resourceLoadingRequest:request];
    [self.pendingRequestWorkers addObject:requestWorker];
    requestWorker.delegate = self;
    [requestWorker startWork];
...
- (NSError *)loaderCancelledError {
    NSError *error = [[NSError alloc] initWithDomain:MCResourceLoaderErrorDomainE
                                                code:-3
