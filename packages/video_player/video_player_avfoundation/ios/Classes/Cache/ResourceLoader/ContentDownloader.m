...
- (void)actionWorker:(ActionWorker *)actionWorker didReceiveData:(NSData *)data isLocal:(BOOL)isLocal;
...
- (instancetype)initWithActions:(NSArray<CacheAction *> *)actions url:(NSURL *)url cacheWorker:(ContentCacheWorker *)cacheWorker;
...
- (NSURLSession *)session {
    if (!_session) {
        ...
        NSURLSession *session = [NSURLSession sessionWithConfiguration:configuration delegate:self.sessionDelegateObject delegateQueue:[CacheSessionManager shared].downloadQueue];
        _session = session;
    }
    return _session;
}
...
- (void)downloadTaskFromOffset:(unsigned long long)fromOffset
    ...
    self.actionWorker = [[ActionWorker alloc] initWithActions:actions url:self.url cacheWorker:self.cacheWorker];
