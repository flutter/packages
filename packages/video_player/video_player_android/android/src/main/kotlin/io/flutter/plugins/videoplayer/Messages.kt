...
data class ReloadingStartEvent (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): ReloadingStartEvent {
      val placeholder = pigeonVar_list[0] as Boolean
      return ReloadingStartEvent(placeholder)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is ReloadingStartEvent) {
      return false
    }
    if (this === other) {
      return true
...
data class ReloadingEndEvent (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): ReloadingEndEvent {
      val duration = pigeonVar_list[0] as Long
      val width = pigeonVar_list[1] as Long
      val height = pigeonVar_list[2] as Long
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      135.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          ReloadingStartEvent.fromList(it)
        }
      }
      136.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
...
interface AndroidVideoPlayerApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: AndroidVideoPlayerApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.AndroidVideoPlayerApi.load$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val msgArg = args[0] as LoadMessage
            val wrapped: List<Any?> = try {
              api.load(msgArg)
              listOf(null)
            } catch (exception: Throwable) {
