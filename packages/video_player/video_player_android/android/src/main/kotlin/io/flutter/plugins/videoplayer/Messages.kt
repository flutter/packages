...
interface VideoPlayerInstanceApi {
   ...
   */
  fun setBandwidthLimit(maxBandwidthBps: Long)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: VideoPlayerInstanceApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.setBandwidthLimit$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val maxBandwidthBpsArg = args[0] as Long
            val wrapped: List<Any?> = try {
              api.setBandwidthLimit(maxBandwidthBpsArg)
              listOf(null)
            } catch (exception: Throwable) {
              MessagesPigeonUtils.wrapError(exception)
            }
