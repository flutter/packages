...
void main() {
  ...
  group('Event Tests', () {
    ...
    test('Should receive device orientation change events', () async {
      ...
      for (int i = 0; i < 3; i++) {
        await _ambiguate(TestDefaultBinaryMessengerBinding.instance)!.defaultBinaryMessenger.handlePlatformMessage(
            AVFoundationCamera.deviceEventChannelName,
  ...
  group('Function Tests', () {
    ...
    test('Should take a picture and return an XFile instance with jpeg format', () async {
      // Arrange
    ...
    test('Should take a picture and return an XFile instance with heic format', () async {
      // Arrange
      final MethodChannelMock channel =
          MethodChannelMock(channelName: _channelName, methods: <String, dynamic>{'takePicture': '/test/path.heic'});

      // Act
      final XFile file = await camera.takePicture(cameraId, imageExtension: ImageExtension.heic);
      ...
      expect(file.path, '/test/path.heic');
    });
