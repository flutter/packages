...
PlatformFrameData::PlatformFrameData(const EncodableList& data, int64_t width,
                                     int64_t height, int64_t length)
    : data_(data), width_(width), height_(height), length_(length) {}

const EncodableList& PlatformFrameData::data() const { return data_; }

void PlatformFrameData::set_data(const EncodableList& value_arg) {
  data_ = value_arg;
}

int64_t PlatformFrameData::width() const { return width_; }

void PlatformFrameData::set_width(int64_t value_arg) { width_ = value_arg; }

int64_t PlatformFrameData::height() const { return height_; }

void PlatformFrameData::set_height(int64_t value_arg) { height_ = value_arg; }

int64_t PlatformFrameData::length() const { return length_; }

void PlatformFrameData::set_length(int64_t value_arg) { length_ = value_arg; }

EncodableList PlatformFrameData::ToEncodableList() const {
  EncodableList list;
  list.reserve(4);
  list.push_back(EncodableValue(data_));
  list.push_back(EncodableValue(width_));
  list.push_back(EncodableValue(height_));
  list.push_back(EncodableValue(length_));
  return list;
}

PlatformFrameData PlatformFrameData::FromEncodableList(
    const EncodableList& list) {
  PlatformFrameData decoded(
      std::get<EncodableList>(list[0]), std::get<int64_t>(list[1]),
...
EncodableValue PigeonInternalCodecSerializer::ReadValueOfType(
  ...
  switch (type) {
    ...
    case 132: {
      return CustomEncodableValue(PlatformFrameData::FromEncodableList(
          std::get<EncodableList>(ReadValue(stream))));
    }
    default:
...
void PigeonInternalCodecSerializer::WriteValue(
  ...
  if (const CustomEncodableValue* custom_value =
    ...
    if (custom_value->type() == typeid(PlatformFrameData)) {
      stream->WriteByte(132);
      WriteValue(EncodableValue(std::any_cast<PlatformFrameData>(*custom_value)
                                    .ToEncodableList()),
                 stream);
      return;
    }
