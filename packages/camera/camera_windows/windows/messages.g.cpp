    ...
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const ::flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          const auto* video_output_path_arg = std::get_if<std::string>(&encodable_video_output_path_arg);
          api->StartVideoRecording(camera_id_arg, video_output_path_arg, [reply](std::optional<FlutterError>&& output) {
            if (output.has_value()) {
              reply(WrapError(output.value()));
              return;
            }
