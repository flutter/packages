...
PlatformMediaSettings PlatformMediaSettings::FromEncodableList(const EncodableList& list) {
  ...
  if (!encodable_frames_per_second.IsNull()) {
    decoded.set_frames_per_second(std::get<int64_t>(encodable_frames_per_second));
  }
...
bool PlatformMediaSettings::operator==(const PlatformMediaSettings& other) const {
  return PigeonInternalDeepEquals(resolution_preset_, other.resolution_preset_) && PigeonInternalDeepEquals(frames_per_second_, other.frames_per_second_) && PigeonInternalDeepEquals(video_bitrate_, other.video_bitrate_) && PigeonInternalDeepEquals(audio_bitrate_, other.audio_bitrate_) && PigeonInternalDeepEquals(enable_audio_, other.enable_audio_);
}
...
std::ostream& operator<<(
  std::ostream& os,
  const PlatformMediaSettings& obj) {
  os << "PlatformMediaSettings(";
  os << "resolution_preset: ";
  os << PigeonInternalToString(obj.resolution_preset_);
  os << ", frames_per_second: ";
  if (obj.frames_per_second_) {
    os << PigeonInternalToString(*obj.frames_per_second_);
  }
  else {
    os << "null";
  }
  os << ", video_bitrate: ";
  if (obj.video_bitrate_) {
    os << PigeonInternalToString(*obj.video_bitrate_);
  }
  else {
    os << "null";
  }
  os << ", audio_bitrate: ";
  if (obj.audio_bitrate_) {
    os << PigeonInternalToString(*obj.audio_bitrate_);
  }
  else {
    os << "null";
  }
  os << ", enable_audio: ";
  os << PigeonInternalToString(obj.enable_audio_);
  os << ")";
  return os;
}
...
size_t PigeonInternalDeepHash(const PlatformSize& v) {
  return v.Hash();
}
...
void CameraApi::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.camera_windows.CameraApi.getAvailableCameras" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.camera_windows.CameraApi.create" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const ::flutter::MessageReply<EncodableValue>& reply) {
        try {
...
void CameraEventApi::CameraClosing(
  ...
  const std::string channel_name = "dev.flutter.pigeon.camera_windows.CameraEventApi.cameraClosing" + message_channel_suffix_;
...
void CameraEventApi::Error(
  ...
  std::function<void(void)>&& on_success,
  std::function<void(const FlutterError&)>&& on_error) {
  const std::string channel_name = "dev.flutter.pigeon.camera_windows.CameraEventApi.error" + message_channel_suffix_;
  BasicMessageChannel<> channel(binary_messenger_, channel_name, &GetCodec());
