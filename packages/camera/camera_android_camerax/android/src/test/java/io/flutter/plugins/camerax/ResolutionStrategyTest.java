...
public class ResolutionStrategyTest {
  ...
  public void pigeon_defaultConstructor() {
    ...
    final ResolutionStrategy resolutionStrategy = api.pigeon_defaultConstructor(size, ResolutionStrategyFallbackRule.CLOSEST_HIGHER);

    assertEquals(resolutionStrategy.getBoundSize(), size);
    assertEquals(resolutionStrategy.getFallbackRule(), ResolutionStrategy.FALLBACK_RULE_CLOSEST_HIGHER);
  }
  ...
  public void getFallbackRule() {
    ...
    final ResolutionStrategy instance = mock(ResolutionStrategy.class);;
    when(instance.getFallbackRule()).thenReturn(ResolutionStrategy.FALLBACK_RULE_CLOSEST_HIGHER_THEN_LOWER);

    assertEquals(ResolutionStrategyFallbackRule.CLOSEST_HIGHER_THEN_LOWER, api.getFallbackRule(instance ));
  }
}
