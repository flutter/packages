...
abstract class PigeonApiCameraSelector(open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar) {
  abstract fun pigeon_defaultConstructor(requireLensFacing: LensFacing?, cameraInfo: androidx.camera.core.CameraInfo?): androidx.camera.core.CameraSelector
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val wrapped: List<Any?> = try {
              api.pigeonRegistrar.instanceManager.addDartCreatedInstance(api.pigeon_defaultConstructor(requireLensFacingArg,cameraInfoArg), pigeon_identifierArg)
              listOf(null)
            } catch (exception: Throwable) {
              CameraXLibraryPigeonUtils.wrapError(exception)
            }
