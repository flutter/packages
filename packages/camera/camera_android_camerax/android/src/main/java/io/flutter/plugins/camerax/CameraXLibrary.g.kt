...
abstract class PigeonApiImageAnalysis(open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar) {
  abstract fun pigeon_defaultConstructor(resolutionSelector: androidx.camera.core.resolutionselector.ResolutionSelector?, outputImageFormat: Long?, targetRotation: Long?): androidx.camera.core.ImageAnalysis
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val wrapped: List<Any?> = try {
              api.pigeonRegistrar.instanceManager.addDartCreatedInstance(api.pigeon_defaultConstructor(resolutionSelectorArg,outputImageFormatArg,targetRotationArg), pigeon_identifierArg)
              listOf(null)
            } catch (exception: Throwable) {
              CameraXLibraryPigeonUtils.wrapError(exception)
            }
