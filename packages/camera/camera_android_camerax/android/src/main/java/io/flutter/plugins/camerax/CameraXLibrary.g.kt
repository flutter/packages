  ...
  fun pigeon_newInstance(pigeon_instanceArg: androidx.camera.core.CameraInfo, callback: (Result<Unit>) -> Unit)
{
    if (pigeonRegistrar.ignoreCallsToDart) {
      ...
      val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
      channel.send(listOf(pigeon_identifierArg, sensorRotationDegreesArg, lensFacingArg, exposureStateArg)) {
        if (it is List<*>) {
          if (it.size > 1) {
            callback(Result.failure(CameraXError(it[0] as String, it[1] as String, it[2] as String?)))
          } else {
  ...
  fun pigeon_newInstance(pigeon_instanceArg: androidx.camera.core.resolutionselector.ResolutionSelector, callback: (Result<Unit>) -> Unit)
{
    if (pigeonRegistrar.ignoreCallsToDart) {
      ...
      val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
      channel.send(listOf(pigeon_identifierArg, resolutionFilterArg, resolutionStrategyArg, allowedResolutionModeArg)) {
        if (it is List<*>) {
          if (it.size > 1) {
            callback(Result.failure(CameraXError(it[0] as String, it[1] as String, it[2] as String?)))
          } else {
...
abstract class PigeonApiCaptureRequest(open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar) {
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiCaptureRequest?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.camera_android_camerax.CaptureRequest.controlVideoStabilizationMode", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_identifierArg = args[0] as Long
            val wrapped: List<Any?> = try {
              api.pigeonRegistrar.instanceManager.addDartCreatedInstance(api.controlVideoStabilizationMode(), pigeon_identifierArg)
              listOf(null)
            } catch (exception: Throwable) {
              CameraXLibraryPigeonUtils.wrapError(exception)
            }
...
abstract class PigeonApiCameraCharacteristics(open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar) {
   ...
   * This key is available on all devices.
   */
  abstract fun controlAvailableVideoStabilizationModes(): android.hardware.camera2.CameraCharacteristics.Key<*>
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiCameraCharacteristics?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.camera_android_camerax.CameraCharacteristics.controlAvailableVideoStabilizationModes", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_identifierArg = args[0] as Long
            val wrapped: List<Any?> = try {
              api.pigeonRegistrar.instanceManager.addDartCreatedInstance(api.controlAvailableVideoStabilizationModes(), pigeon_identifierArg)
              listOf(null)
            } catch (exception: Throwable) {
              CameraXLibraryPigeonUtils.wrapError(exception)
            }
...
abstract class PigeonApiPreviewView(open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar) {
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiPreviewView?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.camera_android_camerax.PreviewView.getMeteringPointFactory", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as androidx.camera.view.PreviewView
