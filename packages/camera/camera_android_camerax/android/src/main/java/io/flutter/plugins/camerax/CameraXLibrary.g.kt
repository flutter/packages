...
abstract class CameraXLibraryPigeonProxyApiRegistrar(val binaryMessenger: BinaryMessenger) {
  ...
  /**
   * An implementation of [PigeonApiPreviewView] used to add a new Dart instance of `PreviewView` to
   * the Dart `InstanceManager`.
   */
...
private open class CameraXLibraryPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      140.toByte() -> {
        return (readValue(buffer) as Long?)?.let { ImplementationMode.ofRaw(it.toInt()) }
      }
      else -> super.readValueOfType(type, buffer)
    }
  }
...
abstract class PigeonApiPreviewView(
  ...
  abstract fun getMeteringPointFactory(
      pigeon_instance: androidx.camera.view.PreviewView
  ): androidx.camera.core.MeteringPointFactory

  abstract fun setImplementationMode(
      pigeon_instance: androidx.camera.view.PreviewView,
  ...
  fun pigeon_newInstance(
      pigeon_instanceArg: androidx.camera.view.PreviewView,
      callback: (Result<Unit>) -> Unit
  ) {
    if (pigeonRegistrar.ignoreCallsToDart) {
      callback(
...
@Suppress("UNCHECKED_CAST")
open class PigeonApiSurfaceProvider(
    open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar
) {
  @Suppress("LocalVariableName", "FunctionName")
  /** Creates a Dart instance of SurfaceProvider and attaches it to [pigeon_instanceArg]. */
  fun pigeon_newInstance(
      pigeon_instanceArg: androidx.camera.core.Preview.SurfaceProvider,
      callback: (Result<Unit>) -> Unit
  ) {
    if (pigeonRegistrar.ignoreCallsToDart) {
      callback(
