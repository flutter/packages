...
abstract class CameraXLibraryPigeonProxyApiRegistrar(val binaryMessenger: BinaryMessenger) {
  ...
  /**
   * An implementation of [PigeonApiPlaneProxyUtils] used to add a new Dart instance of
   * `PlaneProxyUtils` to the Dart `InstanceManager`.
   */
  abstract fun getPigeonApiPlaneProxyUtils(): PigeonApiPlaneProxyUtils
  ...
  fun setUp() {
    ...
    PigeonApiPlaneProxyUtils.setUpMessageHandlers(binaryMessenger, getPigeonApiPlaneProxyUtils())
  }
  fun tearDown() {
    ...
    PigeonApiPlaneProxyUtils.setUpMessageHandlers(binaryMessenger, null)
  }
}
...
/** TODO(camsim99) */
@Suppress("UNCHECKED_CAST")
abstract class PigeonApiPlaneProxyUtils(open val pigeonRegistrar: CameraXLibraryPigeonProxyApiRegistrar) {
  /** TODO(camsim99) */
  abstract fun getNv21Plane(planeProxyList: List<androidx.camera.core.ImageProxy.PlaneProxy>, imageWidth: Long, imageHeight: Long): ByteArray

  companion object {
    @Suppress("LocalVariableName")
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiPlaneProxyUtils?) {
      val codec = api?.pigeonRegistrar?.codec ?: CameraXLibraryPigeonCodec()
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.camera_android_camerax.PlaneProxyUtils.getNv21Plane", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val planeProxyListArg = args[0] as List<androidx.camera.core.ImageProxy.PlaneProxy>
            val imageWidthArg = args[1] as Long
            val imageHeightArg = args[2] as Long
            val wrapped: List<Any?> = try {
              listOf(api.getNv21Plane(planeProxyListArg, imageWidthArg, imageHeightArg))
            } catch (exception: Throwable) {
              CameraXLibraryPigeonUtils.wrapError(exception)
            }
            reply.reply(wrapped)
  ...
  fun pigeon_newInstance(pigeon_instanceArg: androidx.camera.core.PlaneProxyUtils, callback: (Result<Unit>) -> Unit)
{
    if (pigeonRegistrar.ignoreCallsToDart) {
      ...
      val binaryMessenger = pigeonRegistrar.binaryMessenger
      val codec = pigeonRegistrar.codec
      val channelName = "dev.flutter.pigeon.camera_android_camerax.PlaneProxyUtils.pigeon_newInstance"
      val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
      channel.send(listOf(pigeon_identifierArg)) {
        if (it is List<*>) {
          if (it.size > 1) {
