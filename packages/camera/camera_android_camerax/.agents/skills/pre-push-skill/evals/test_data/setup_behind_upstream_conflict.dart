...
void main() async {
  final Directory packageDir = Directory(Platform.script.toFilePath()).parent.parent.parent.parent.parent.parent;
  ...
  final ProcessResult fetchResult = await Process.run(
    'git',
    <String>['fetch', '--depth=2', upstream, 'main'],
  ...
  final ProcessResult diffResult = await Process.run(
    'git',
    <String>['diff', '$upstream/main~1..$upstream/main', '--name-only'],
    workingDirectory: packageDir.path,
  );
  ...
  final ProcessResult checkoutResult = await Process.run(
    'git',
    <String>['checkout', '-B', 'eval_behind_upstream_conflict', '$upstream/main~1'],
    workingDirectory: packageDir.path,
  );
  ...
  final File file = File('${packageDir.path}/$fileToConflict');
  ...
  final ProcessResult commitResult = await Process.run(
    'git',
    <String>[
      '-c',
      'user.name=Author',
