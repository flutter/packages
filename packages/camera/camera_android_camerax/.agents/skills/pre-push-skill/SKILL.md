...
```bash
dart run .agents/skills/pre-push-skill/tool/check_eval_commits.dart --remote=origin
```

*Note: You can optionally specify `--base-branch=<branch>` (defaults to `main`) or `--head=<ref>` (defaults to `HEAD`), or a different `--remote=<remote>` if comparing against a custom upstream repository or branch.*
