...
protocol PigeonApiDelegateWKHTTPCookieStore {
  ...
  func getCookies(pigeonApi: PigeonApiWKHTTPCookieStore, pigeonInstance: WKHTTPCookieStore, domain: String?, completion: @escaping (Result<[HTTPCookie], Error>) -> Void)
}
...
final class PigeonApiWKHTTPCookieStore: PigeonApiProtocolWKHTTPCookieStore  {
  ...
  static func setUpMessageHandlers(binaryMessenger: FlutterBinaryMessenger, api: PigeonApiWKHTTPCookieStore?) {
    ...
    let getCookiesChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.webview_flutter_wkwebview.WKHTTPCookieStore.getCookies", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getCookiesChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let pigeonInstanceArg = args[0] as! WKHTTPCookieStore
        let domainArg: String? = nilOrValue(args[1])
        api.pigeonDelegate.getCookies(pigeonApi: api, pigeonInstance: pigeonInstanceArg, domain: domainArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
