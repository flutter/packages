...
/**
 * AndroidLocalNetwork handles the ACCESS_LOCAL_NETWORK permission. It can be used via jnigen to
 * check and request permissions from Dart.
 */
public class AndroidLocalNetworkPlugin
  ...
  private final List<PermissionCallback> pendingCallbacks = new ArrayList<>();
  ...
  public void requestPermission(PermissionCallback callback) {
    ...
    synchronized (pendingCallbacks) {
      ...
      if (pendingCallbacks.size() > 1) {
        Log.d(TAG, "requestPermission: Request already in progress, queuing callback.");
        return;
      }
