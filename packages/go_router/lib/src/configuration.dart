        ...
        for (final StatefulShellBranch branch in route.branches) {
          if (branch.initialLocation == null) {
            ...
            final String? initialLocation =
                defaultGoRoute != null ? locationForRoute(defaultGoRoute) : null;
            assert(
    ...
    try {
      ...
      return routeLevelRedirectResult.then<RouteMatchList>(processRouteLevelRedirect).catchError((
        ...
        final GoException goException =
            error is GoException ? error : GoException('Exception during route redirect: $error');
  ...
  FutureOr<RouteMatchList> applyTopLegacyRedirect(
    ...
    try {
      ...
      return res.then<RouteMatchList>(done).catchError((Object error) {
        final GoException goException =
            error is GoException ? error : GoException('Exception during redirect: $error');
