...
/// in that they accept a [_StatefulNavigationShell] parameter instead of a
...
/// accomplished by using the method [_StatefulNavigationShell.goBranch], for
/// example:
///
/// ```
...
class StatefulShellRoute extends ShellRouteBase {
  ...
  /// Widget.
  ///
  /// The state parameter of this function ([ShellRouteState]) can also be used
  /// to access information about which branch is active (see
  /// [ShellRouteState.navigatorIndex]), and also to navigate to a different
  /// branch (using [ShellRouteState.restoreNavigator]).
  final ShellRouteBuilder? builder;
  ...
  final GlobalKey _shellStateKey;

  _StatefulNavigationShellState? get _shellState =>
      _shellStateKey.currentState as _StatefulNavigationShellState?;
  ...
  RouteMatchList locationOfNavigator(GoRouterState state, int navigatorIndex) {
    final RouteMatchList? matchList =
        _shellState?._matchListForBranch(navigatorIndex);
    if (matchList != null && matchList.isNotEmpty) {
      return matchList;
    } else {
      final String initialLocation =
          branches[navigatorIndex]._effectiveInitialBranchLocation(state);
      return state.findMatch(Uri.parse(initialLocation));
    }
  }

  @override
  void resetLocationOfNavigator(ShellRouteState state, int navigatorIndex) {
    _shellState?._resetMatchListForBranch(navigatorIndex);
  }
  ...
  static ShellRouteState? shellRouteStateForKey(
          GlobalKey key) =>
      (key.currentState as _StatefulNavigationShellState?)?._routeState;

  /// Get the initial location for the branch at the given index.
  ///
  /// See also [StatefulShellBranch.initialLocation]).
  String initialBranchLocation(ShellRouteState state, int branchIndex) =>
      branches[branchIndex]._effectiveInitialBranchLocation(state);
  ...
  static Widget _indexedStackContainerBuilder(
      BuildContext context, ShellRouteState state, List<Widget> children) {
    return _IndexedStackedRouteBranchContainer(
        currentIndex: state.navigatorIndex, children: children);
  }
...
/// using the goBranch method in [_StatefulNavigationShell]).
///
...
class StatefulShellBranch {
  ...
  String _effectiveInitialBranchLocation(GoRouterState state) {
    if (initialLocation != null) {
      ...
      final String fullPath = state.locationForRoute(route)!;
      return patternToPath(fullPath, state.pathParameters);
    }
  }
}
...
class _StatefulNavigationShell extends StatefulWidget {
  ...
  final ShellNavigationContainerBuilder _containerBuilder;

  @override
  State<StatefulWidget> createState() => _StatefulNavigationShellState();
}

/// State for [_StatefulNavigationShell], and consequently [StatefulShellRoute].
class _StatefulNavigationShellState extends State<_StatefulNavigationShell>
  ...
  ShellRouteState get _routeState => widget._routeState;

  /// The associated [StatefulShellRoute].
  StatefulShellRoute get _route => _routeState.shellRoute as StatefulShellRoute;
  ...
  void _updateCurrentBranchStateFromWidget() {
    final StatefulShellBranch branch =
        _route.branches[_routeState.navigatorIndex];
    final RouteMatchList currentBranchLocation =
        _scopedMatchList(_routeState.routeMatchList);
    ...
    if (locationChanged || !hasExistingNavigator) {
      _branchNavigators[branch.navigatorKey] =
          widget._navigatorBuilder(branch.observers, branch.restorationScopeId);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return widget._containerBuilder(context, _routeState, children);
  }
}
