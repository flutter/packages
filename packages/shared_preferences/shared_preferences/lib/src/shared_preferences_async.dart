...
class SharedPreferencesAsync {
  ...
  /// Reads a value from the platform, throwing an exception if the value is not a
  /// bool.
  Future<bool?> getBool(String key) async => _platform.getBool(key, _options);

  /// Reads a value from the platform, throwing an exception if the value is not
  /// an int.
  ...
  Future<List<String>?> getStringList(String key) async {
    List<String?>? list = await _platform.getStringList(key, _options);
  ...
  ///
  /// It is highly recommended that [PreferencesFilters] be provided to this call.
...
class SharedPreferencesWithCache {
  ...
  bool _isValidKey(String key) {
    return _cacheOptions.filter.allowList?.contains(key) ?? true;
  }
}
