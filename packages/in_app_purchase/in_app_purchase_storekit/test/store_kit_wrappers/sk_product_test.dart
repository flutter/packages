...
void main() {
  group('product related object wrapper test', () {
    test('SKProductSubscriptionPeriodWrapper should have property values consistent with map', () {
    ...
    test(
      ...
      () {
        final wrapper = SKProductSubscriptionPeriodWrapper.fromJson(const <String, dynamic>{});
        expect(wrapper.numberOfUnits, 0);
        expect(wrapper.unit, SKSubscriptionPeriodUnit.day);
      },
    );

    test('SKProductDiscountWrapper should have property values consistent with map', () {
      final wrapper = SKProductDiscountWrapper.fromJson(buildDiscountMap(dummyDiscount));
    ...
    test('SKProductDiscountWrapper missing identifier and type should have '
      ...
      final wrapper = SKProductDiscountWrapper.fromJson(
        buildDiscountMapMissingIdentifierAndType(dummyDiscountMissingIdentifierAndType),
      );
      expect(wrapper, equals(dummyDiscountMissingIdentifierAndType));
    });

    test('SKProductDiscountWrapper should have properties to be default if map is empty', () {
      final wrapper = SKProductDiscountWrapper.fromJson(const <String, dynamic>{});
      expect(wrapper.price, '');
      expect(
        wrapper.priceLocale,
        SKPriceLocaleWrapper(currencyCode: '', currencySymbol: '', countryCode: ''),
      );
    ...
    test('SKProductWrapper should have property values consistent with map', () {
      final wrapper = SKProductWrapper.fromJson(buildProductMap(dummyProductWrapper));
    ...
    test('SKProductResponse wrapper should default to empty list', () {
      ...
      final wrapper = SkProductResponseWrapper.fromJson(productResponseMapEmptyList);
    ...
    test('LocaleWrapper should have property values consistent with map', () {
      final wrapper = SKPriceLocaleWrapper.fromJson(buildLocaleMap(dollarLocale));
  ...
  group('Payment queue related object tests', () {
    ...
    test('SKPaymentWrapper should have propery values consistent with .toMap()', () {
      ...
      expect(mapResult['productIdentifier'], dummyPaymentWithDiscount.productIdentifier);
      expect(mapResult['applicationUsername'], dummyPaymentWithDiscount.applicationUsername);
    ...
    test('toPurchaseDetails() should return correct PurchaseDetail object', () {
      final details = AppStorePurchaseDetails.fromSKTransaction(dummyTransaction, 'receipt data');
    ...
    test('Should generate correct map of the payment object', () {
      ...
      expect(map['simulatesAskToBuyInSandbox'], dummyPayment.simulatesAskToBuyInSandbox);
    });
  });
}
