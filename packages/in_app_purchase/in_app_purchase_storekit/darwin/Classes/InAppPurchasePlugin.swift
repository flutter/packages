...
public class InAppPurchasePlugin: NSObject, FlutterPlugin, InAppPurchaseAPI {
  ...
  public convenience init(registrar: FlutterPluginRegistrar) {
    ...
    transactionObserverCallbackChannel = FlutterMethodChannel(name: "plugins.flutter.io/in_app_purchase", binaryMessenger: registrar.messenger())
  }
...
//  }
  ...
  public func addPaymentPaymentMap(_ paymentMap: [String : Any], error: AutoreleasingUnsafeMutablePointer<FlutterError?>) {
        ...
        guard let product = self.getProduct(productID: productID) else {
          error.pointee = FlutterError(code: "storekit_invalid_payment_object",
                               message: "You have requested a payment for an invalid product. Either the `productIdentifier` of the payment is not valid or the product has not been fetched before adding the payment to the payment queue.",
                               details: paymentMap)
          return;
        }
  ...
  public func finishTransactionFinishMap(_ finishMap: [String: Any], error: AutoreleasingUnsafeMutablePointer<FlutterError?>) {
      guard let transactionIdentifier = finishMap["transactionIdentifier"] as? String,
            let productIdentifier = finishMap["productIdentifier"] as? String else {
          return
      }
