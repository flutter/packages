...
- (NSArray *)toList {
  return @[
    ...
    (self.simulatesAskToBuyInSandbox ?: [NSNull null]),
...
+ (SKPaymentDiscountMessage *)fromList:(NSArray *)list {
  ...
  NSAssert(pigeonResult.keyIdentifier != nil, @"");
  pigeonResult.nonce = GetNullableObjectAtIndex(list, 2);
  NSAssert(pigeonResult.nonce != nil, @"");
  pigeonResult.signature = GetNullableObjectAtIndex(list, 3);
  NSAssert(pigeonResult.signature != nil, @"");
...
void InAppPurchaseAPISetup(id<FlutterBinaryMessenger> binaryMessenger,
  ...
  }
  {
    FlutterBasicMessageChannel *channel = [[FlutterBasicMessageChannel alloc]
        ...
        binaryMessenger:binaryMessenger
                  codec:InAppPurchaseAPIGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(addPayment:error:)],
                @"InAppPurchaseAPI api (%@) doesn't respond to @selector(addPayment:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
