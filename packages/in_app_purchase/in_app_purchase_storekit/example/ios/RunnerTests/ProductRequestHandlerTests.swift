...
class RequestHandlerTest: XCTestCase {
    ...
    func testRequestHandlerWithProductRequestFailure() {
        let request = SKProductRequestStub(failureError: NSError(domain: "test", code: 123, userInfo: [:]))
        let handler = FIAPRequestHandler(request: request)
    ...
    func testRequestHandlerWithRefreshReceiptFailure() {
        let request = SKReceiptRefreshRequestStub(failureError: NSError(domain: "test", code: 123, userInfo: [:]))
        let handler = FIAPRequestHandler(request: request)
