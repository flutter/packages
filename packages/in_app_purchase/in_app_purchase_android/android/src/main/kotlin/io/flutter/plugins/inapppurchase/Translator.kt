...
/**
 * Handles serialization and deserialization of [BillingClient]
 * related objects.
 */
/*package*/
internal object Translator {
    ...
    fun toProduct(
        platformProduct: PlatformQueryProduct
    ...
    fun toPlatformRecurrenceMode(mode: Int): PlatformRecurrenceMode {
        when (mode) {
            ...
            ProductDetails.RecurrenceMode.INFINITE_RECURRING -> return PlatformRecurrenceMode.INFINITE_RECURRING
    ...
    fun fromPurchase(purchase: Purchase): PlatformPurchase {
        ...
        val billingPendingPurchaseUpdate =
    ...
    fun fromBillingResult(billingResult: BillingResult): PlatformBillingResult {
        return PlatformBillingResult(
            fromBillingResponseCode(billingResult.getResponseCode()),
    ...
    fun fromBillingResponseCode(billingResponseCode: Int): PlatformBillingResponse {
        when (billingResponseCode) {
            BillingClient.BillingResponseCode.FEATURE_NOT_SUPPORTED -> return PlatformBillingResponse.FEATURE_NOT_SUPPORTED
            BillingClient.BillingResponseCode.SERVICE_DISCONNECTED -> return PlatformBillingResponse.SERVICE_DISCONNECTED
            ...
            BillingClient.BillingResponseCode.SERVICE_UNAVAILABLE -> return PlatformBillingResponse.SERVICE_UNAVAILABLE
            BillingClient.BillingResponseCode.BILLING_UNAVAILABLE -> return PlatformBillingResponse.BILLING_UNAVAILABLE
            BillingClient.BillingResponseCode.ITEM_UNAVAILABLE -> return PlatformBillingResponse.ITEM_UNAVAILABLE
            BillingClient.BillingResponseCode.DEVELOPER_ERROR -> return PlatformBillingResponse.DEVELOPER_ERROR
            BillingClient.BillingResponseCode.ERROR -> return PlatformBillingResponse.ERROR
            BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED -> return PlatformBillingResponse.ITEM_ALREADY_OWNED
            BillingClient.BillingResponseCode.ITEM_NOT_OWNED -> return PlatformBillingResponse.ITEM_NOT_OWNED
    ...
    fun fromUserChoiceProduct(
        product: UserChoiceDetails.Product
    ...
    fun fromBillingConfig(
        result: BillingResult, billingConfig: BillingConfig?
    ): PlatformBillingConfigResponse {
        return PlatformBillingConfigResponse(
            fromBillingResult(result),
            if (billingConfig == null) "" else billingConfig.getCountryCode()
        )
    }
    ...
    fun fromAlternativeBillingOnlyReportingDetails(
        ...
        return PlatformAlternativeBillingOnlyReportingDetailsResponse(
            fromBillingResult(result),
            if (details == null) "" else details.getExternalTransactionToken()
        )
    }
    ...
    fun toBillingClientFeature(feature: PlatformBillingClientFeature): String {
        when (feature) {
            PlatformBillingClientFeature.ALTERNATIVE_BILLING_ONLY -> return BillingClient.FeatureType.ALTERNATIVE_BILLING_ONLY
            ...
            PlatformBillingClientFeature.IN_APP_MESSAGING -> return BillingClient.FeatureType.IN_APP_MESSAGING
            PlatformBillingClientFeature.PRICE_CHANGE_CONFIRMATION -> return BillingClient.FeatureType.PRICE_CHANGE_CONFIRMATION
            ...
            PlatformBillingClientFeature.SUBSCRIPTIONS_UPDATE -> return BillingClient.FeatureType.SUBSCRIPTIONS_UPDATE
        }
    ...
    fun toReplacementMode(replacementMode: PlatformReplacementMode): Int {
        when (replacementMode) {
            PlatformReplacementMode.CHARGE_FULL_PRICE -> return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.CHARGE_FULL_PRICE
            PlatformReplacementMode.CHARGE_PRORATED_PRICE -> return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.CHARGE_PRORATED_PRICE
            PlatformReplacementMode.DEFERRED -> return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.DEFERRED
            PlatformReplacementMode.WITHOUT_PRORATION -> return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.WITHOUT_PRORATION
            PlatformReplacementMode.WITH_TIME_PRORATION -> return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.WITH_TIME_PRORATION
            PlatformReplacementMode.UNKNOWN_REPLACEMENT_MODE -> return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.UNKNOWN_REPLACEMENT_MODE
        }
        return BillingFlowParams.SubscriptionUpdateParams.ReplacementMode.UNKNOWN_REPLACEMENT_MODE
    }

    /**
     * Gets the symbol of for the given currency code for the default [ DISPLAY][Locale.Category.DISPLAY] locale. For example, for the US Dollar, the symbol is "$" if the default locale is the
     * US, while for other locales it may be "US$". If no symbol can be determined, the ISO 4217
     * currency code is returned.
     *
     * @param currencyCode the ISO 4217 code of the currency
     * @return the symbol of this currency code for the default [     DISPLAY][Locale.Category.DISPLAY] locale
     * @exception NullPointerException if `currencyCode` is null
     * @exception IllegalArgumentException if `currencyCode` is not a supported ISO 4217
     * code.
     */
    @JvmStatic
