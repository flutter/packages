...
final Finder clickableWrappedButtonFinder = find.byKey(const Key('wrapped-transparent-button'));
  ...
  group('Without semantics', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final web.Element element = _getHtmlElementAtCenter(clickableWrappedButtonFinder, tester);
    ...
    testWidgets('on unwrapped elements, the browser hits the background-html-view', (
      WidgetTester tester,
    ) async {
      await _fullyRenderApp(tester);

      final web.Element element = _getHtmlElementAtCenter(nonClickableButtonFinder, tester);
    ...
    testWidgets('on background directly', (WidgetTester tester) async {
      ...
      final web.Element element = _getHtmlElementAt(tester.getTopLeft(backgroundFinder));
    ...
    testWidgets('prevents default action of mousedown events', (WidgetTester tester) async {
      ...
      for (var i = 0; i <= 4; i++) {
        final event = web.MouseEvent('mousedown', web.MouseEventInit(button: i, cancelable: true));
...
web.Element _getHtmlElementAt(Offset point) {
  ...
  final web.ShadowRoot glassPaneShadow = web.document.querySelector('flt-glass-pane')!.shadowRoot!;
